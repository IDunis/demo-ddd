...
from flytrade.configuration import Configuration
from flytrade.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from flytrade.enums import RPCMessageType, State
from flytrade.exceptions import OperationalException, TemporaryError
from flytrade.exchange import timeframe_to_next_date
from flytrade.flytradebot import FlytradeBot
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.flytrade = FlytradeBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.flytrade.state

        # Log state transition
        if state != old_state:

            if old_state != State.RELOAD_CONFIG:
                self.flytrade.notify_status(f'{state.name.lower()}')

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}")
            if state == State.RUNNING:
                self.flytrade.startup()

            if state == State.STOPPED:
                self.flytrade.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.flytrade.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.flytrade.process_stopped()

    def _process_running(self) -> None:
        try:
            self.flytrade.process()
            ...
            self.flytrade.notify_status(
                f'*OperationalException:*\n```\n{tb}```\n {hint}',
                msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception('OperationalException. Stopping trader ...')
            self.flytrade.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current flytradebot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current flytrade modules
        self.flytrade.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.flytrade.notify_status('config reloaded')
    ...
    def exit(self) -> None:
        ...
        if self.flytrade:
            self.flytrade.notify_status('process died')
            self.flytrade.cleanup()
