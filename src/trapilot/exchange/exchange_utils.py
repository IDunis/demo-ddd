...
from trapilot.exchange.common import (BAD_EXCHANGES, EXCHANGE_HAS_OPTIONAL, EXCHANGE_HAS_REQUIRED,
                                       SUPPORTED_EXCHANGES)
from trapilot.types import ValidExchangesType
from trapilot.util import Precise
...
def amount_to_contracts(amount: float, contract_size: Optional[float]) -> float:
    ...
    if contract_size and contract_size != 1:
        return float(Precise(amount) / Precise(contract_size))
    else:
        return amount
...
def contracts_to_amount(num_contracts: float, contract_size: Optional[float]) -> float:
    ...
    if contract_size and contract_size != 1:
        return float(Precise(num_contracts) * Precise(contract_size))
    else:
    ...
    *,
    ...
    if price_precision is not None and precisionMode is not None:
        ...
        if precisionMode == TICK_SIZE:
            precision = Precise(price_precision)
            price_str = Precise(price)
