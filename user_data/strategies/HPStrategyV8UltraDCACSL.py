...
from trapilot.enums import ExitCheckTuple
from trapilot.persistence import Order, Trade
from trapilot.strategy import (BooleanParameter, CategoricalParameter,
...
class HPStrategyV8UltraDCACSL(IStrategy):
    ...
    csl = {}
    minimal_roi = {"0": 0.015 * leverage_value}
    ...
    custom_tp_pct = DecimalParameter(
        0.005, 0.50, default=0.005, decimals=3, space="sell", optimize=False
    )

    dca_threshold_pct_k = DecimalParameter(
        0.90,
        0.99,
        default=0.94,
        decimals=2,
    ...
    dca_threshold_pct = DecimalParameter(
        0.1,
        0.5,
        default=0.05,
        decimals=2,
    ...
    dca_multiplier = DecimalParameter(
        1.2, 10, default=3, decimals=2, space="buy", optimize=position_adjustment_enable
    )

    dca_limit = IntParameter(
        1, 5, default=1, space="buy", optimize=position_adjustment_enable
    )
    ...
    kick_off_threshold = DecimalParameter(
        -0.99, 0, default=-0.32, decimals=2, space="sell", optimize=False
    )

    pct3_buy_threshold = DecimalParameter(
        -0.999, -0.005, default=-0.01, decimals=3, space="buy", optimize=False
    )

    red_candles_before_buy = IntParameter(1, 5, default=1, space="buy", optimize=False)

    candle_time_threshold = DecimalParameter(
        0.01, 1, default=0.36, decimals=2, space="buy", optimize=False
    )

    trade_timeout = IntParameter(1, 48, default=12, space="sell", optimize=False)

    max_tradable_ratio = DecimalParameter(
        0.01, 1, default=0.75, space="buy", optimize=False
    )

    max_trades = IntParameter(
        1, 30, default=max_open_trades, space="buy", optimize=False
    )

    price_change_1 = DecimalParameter(
        0.001, 0.050, default=0.003, decimals=3, space="buy", optimize=True
    )
    price_change_2 = DecimalParameter(
        0.005, 0.150, default=0.025, decimals=3, space="buy", optimize=True
    )
    ...
    def mid_don_cross_over(self, dataframe):
        ...
        dataframe["position_m"] = np.where(
            dataframe["close"] > dataframe["midDon"], 1, dataframe["position_m"]
        )
    ...
    def don_channel_breakout(self, dataframe):
        ...
        dataframe["position_b"] = np.where(
            dataframe["close"] > dataframe["upperDon"].shift(1),
            1,
    ...
    def don_reversal(self, dataframe):
        ...
        dataframe["position_r"] = np.where(
            dataframe["close"] < dataframe["lowerDon"].shift(1),
            1,
    ...
    def calculate_price_change_coefficient(
        self, dataframe: DataFrame, candles_count: int = 1
    ) -> DataFrame:
        dataframe["price_change_pct"] = dataframe["close"].pct_change(
            periods=candles_count
        )
        dataframe["price_change_coeff"] = dataframe["price_change_pct"].apply(
            lambda x: max(min(x, 0.99), -0.99) if x != 0 else 0.01 * np.sign(x)
        )
        dataframe[f"price_change_coeff_{candles_count}"] = (
            dataframe["price_change_coeff"].rolling(window=candles_count).mean()
            * self.leverage_value
        )
        return dataframe
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = self.calc_donchian_channels(
            dataframe=dataframe, period=self.donchian_period.value
        )
        ...
        dataframe["significant_loss"] = (
            dataframe["price_change"] < significant_loss_threshold
        )
        ...
        dataframe["loss_and_recovery_signal"] = (
            (dataframe["significant_loss"] == True)
            & (dataframe["recovery"].shift(-1) == True)
        ).astype(int)
    ...
    def is_candle_open_more_than_threshold(
        self, dataframe: DataFrame, threshold: float
    ) -> DataFrame:
        ...
        dataframe.loc[:, "is_open_more_than_threshold"] = (
            dataframe["time_since_open"] >= t_time
        )
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        self.is_candle_open_more_than_threshold(
            dataframe, threshold=self.candle_time_threshold.value
        )
            ...
            (
                ...
                & (
                    (
                        ...
                        & (
                            dataframe["price_change_coeff_3"]
                            > -self.price_change_2.value
                        )
                    )
                    | (
                        (dataframe["price_change_coeff_3"] > self.price_change_1.value)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[((dataframe["swing_high"] == 1)), ["exit_long", "exit_tag"]] = (
            1,
            "swing_high",
        )
        dataframe.loc[
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        return min(
            max_stake,
            (max_stake * self.max_tradable_ratio.value) / (self.max_trades.value * 1.5),
        )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        last_candle, second_last_candle, third_last_candle = (
            dataframe.iloc[-1],
            dataframe.iloc[-2],
            dataframe.iloc[-3],
        )
        ...
        is_previous_candles_red = all(
            ...
            for candle in [second_last_candle, third_last_candle]
        )
        ...
        if (
            current_profit <= dynamic_threshold
            and is_last_candle_green
            and is_previous_candles_red
        ):
            logging.info(
                f"{current_time} Adjusting position for {trade.pair} with {trade.stake_amount}. "
