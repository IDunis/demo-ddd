...
class HarmonicDivergence(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        HarmonicDivergence.plot_config = (
            ...
            .add_total_divergences_in_config(dataframe).config
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        """
        dataframe.loc[
            ((dataframe["volume"] > 0)), "exit_long"  # Make sure Volume is not 0
        ] = 0
        return dataframe
...
def bollinger_keltner_check(dataframe):
    return (
        dataframe[resample("bollinger_lowerband")] < dataframe[resample("kc_lowerband")]
    ) & (
...
def initialize_divergences_lists(dataframe: DataFrame):
    ...
    dataframe["total_bullish_divergences_count"] = (
        np.empty(len(dataframe["close"])) * np.nan
    )
    dataframe["total_bullish_divergences_count"] = [
    ...
    for index, row in enumerate(dataframe.itertuples(index=True, name="Pandas")):
        ...
        if bearish_occurence != None:
            ...
            if can_exist:
                ...
                if index > 30:
                    index_position = index - 30
                    dataframe.loc[index_position, "total_bearish_divergences_count"] = (
                        dataframe["total_bearish_divergences_count"][index_position] + 1
                    )
                    dataframe.loc[index_position, "total_bearish_divergences_names"] = (
                        str(
                            dataframe["total_bearish_divergences_names"][index_position]
                        )
        ...
        if bullish_occurence != None:
            ...
            if can_exist:
                ...
                if index > 30:
                    index_position = index - 30
                    dataframe.loc[index_position, "total_bullish_divergences_count"] = (
                        dataframe["total_bullish_divergences_count"][index_position] + 1
                    )
                    dataframe.loc[index_position, "total_bullish_divergences_names"] = (
                        str(
                            dataframe["total_bullish_divergences_names"][index_position]
                        )
                        + indicator_source.upper()
                        + "<br>"
                    )
...
def pivot_points(
    dataframe: DataFrame, window: int = 5, pivot_source: PivotSource = PivotSource.Close
) -> DataFrame:
    high_source = None
    low_source = None
