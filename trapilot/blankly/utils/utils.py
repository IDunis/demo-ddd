...
class __BlanklySettings:
    def __init__(
        ...
        Args:
            default_path: The default path to look for settings - such as ./user_data/settings.json
...
def to_trapilot_symbol(symbol, exchange, quote_guess=None) -> str:
    if exchange == "binance":
...
def aggregate_candles(history: pd.DataFrame, aggregation_size: int):
    ...
    Args:
        history: A trapilot generated dataframe
...
class Email:
    """
    Object wrapper for simplifying interaction with SMTP servers & the trapilot.blankly.reporter.email function.
    Alternatively a user_data/notify.json can be created which automatically integrates with trapilot.blankly.reporter.email()
    """
    ...
    def send(self, receiver_email: str, message: str):
        ...
        """
        trapilot.blankly.reporter.email(
            email_str=message,
...
def order_protection(func):
    ...
    def wrapper(*args, **kwargs):
        if trapilot.blankly._backtesting:
            raise Exception(
