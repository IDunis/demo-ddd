...
class BinanceFuturesInterface(FuturesExchangeInterface):
    ...
    def get_leverage(self, symbol: str = None) -> float:
        ...
        return float(
            self.calls.futures_position_information(symbol=symbol)[0]["leverage"]
        )
    ...
    def set_leverage(self, leverage: float, symbol: str = None):
        ...
        symbol = self.to_exchange_symbol(symbol)
        return self.calls.futures_change_leverage(symbol=symbol, leverage=leverage)[
            "maxNotionalValue"
        ]
    ...
    def set_margin_type(self, symbol: str, type: MarginType):
        ...
        try:
            self.calls.futures_change_margin_type(
                symbol=symbol, marginType=type.upper()
            )
        except BinanceAPIException as e:
            if (
                e.code != -4046
            ):  # -4046 NO_NEED_TO_CHANGE_MARGIN_TYPE (margin type is already set)
                raise e
    ...
    def get_position(self, filter: str = None) -> Optional[dict]:
        ...
        for position in account["positions"]:
            ...
            margin = MarginType.ISOLATED if position["isolated"] else MarginType.CROSSED
            positions[symbol] = {
                "symbol": symbol,
                "base_asset": base,
                "quote_asset": quote,
                ...
                "margin_type": (
                    MarginType.ISOLATED
                    if position.get("isolated", False)
    ...
    def parse_order_response(self, response: dict) -> FuturesOrder:
        return FuturesOrder(
            symbol=self.to_blankly_symbol(response["symbol"]),
    ...
    def get_product_history(
        ...
        return BinanceInterface._binance_get_product_history(
    ...
    def get_funding_rate_history(
        ...
        if self.calls.testnet:
            ...
            return [
                {"rate": 0.0001, "time": t} for t in range(start, end + 1, resolution)
            ]
        ...
        while response:
            response = self.calls.futures_funding_rate(
                symbol=symbol, startTime=window, limit=1000
            )
            # very stinky ^^

            history.extend(
                {"rate": float(e["fundingRate"]), "time": e["fundingTime"] // 1000}
                for e in response
            )
