...
def split(base_range, local_segments) -> typing.Tuple[list, list]:
    ...
    for i in range(len(positive_ranges) - 1):
        intersection_range = intersection(
            positive_ranges[i][0],
            positive_ranges[i][1],
            positive_ranges[i + 1][0],
            positive_ranges[i + 1][1],
        )
...
class BackTestController(ABCBacktestController):  # circular import to type model
    ...
    def parse_events(self):
        ...
        for reader in self.__event_readers:
            ...
            for event_type in data:
                self.__check_user_time_bounds(
                    reader.data[event_type]["time"].iloc[0],
        ...
        for tick_reader in self.__tick_readers:
            ...
            for symbol in data:
                self.__check_user_time_bounds(
                    tick_reader.data[symbol]["time"].iloc[0],
    ...
    def sync_prices(self) -> dict:
        ...
        def sort_prices_by_resolution(price_dict):
            for symbol_ in price_dict:
                for resolution_ in price_dict[symbol_]:
                    price_dict[symbol_][resolution_] = price_dict[symbol_][
                        resolution_
        ...
        def sort_identifiers(identifiers_: list) -> dict:
            ...
            def sort_identifier(identifier_: dict):
                ...
                if (
                    resolution_
                    not in local_history_blocks_[exchange_][sandbox_][symbol_]
                ):
                    local_history_blocks_[exchange_][sandbox_][symbol_][
                        resolution_
                    ] = []
                ...
                local_history_blocks_[exchange_][sandbox_][symbol_][resolution_].append(
                    {
                        self.PriceIdentifiers.epoch_start: identifier_[
                            self.PriceIdentifiers.epoch_start
                        ],
                        self.PriceIdentifiers.epoch_stop: identifier_[
                            self.PriceIdentifiers.epoch_stop
                        ],
                    }
                )
        ...
        for i in range(len(self.__user_added_times)):
            ...
            end_time = (
                self.__user_added_times[i][self.PriceIdentifiers.epoch_stop]
                - resolution
            )
            ...
            if end_time < start_time:
                raise RuntimeError(
                    "Must specify a longer timeframe to run the backtest."
                )
            ...
            used_ranges, negative_ranges = split(
                [start_time, end_time], downloaded_ranges
            )
                ...
                relevant_data.append(
                    pd.read_csv(
                        os.path.join(
                            cache_folder,
                            to_string_key(
                                [exchange, True, symbol, j[0], j[1], resolution]
                            )
                            + ".csv",
                        )
                    )
            ...
            if len(relevant_data) > 0:
                ...
                for dataset in relevant_data:
                    prices_by_resolution = aggregate_prices_by_resolution(
                        prices_by_resolution, symbol, resolution, dataset
                    )
            ...
            for j in negative_ranges:
                ...
                download = self.interface.get_product_history(
                    symbol, j[0], j[1], resolution
                )
                ...
                prices_by_resolution = aggregate_prices_by_resolution(
                    prices_by_resolution, symbol, resolution, download
                )
            ...
            final_prices[symbol] = final_prices[symbol].sort_values(
            ...
            final_prices[symbol] = final_prices[symbol][
                final_prices[symbol]["time"] >= start_time
            ]
            final_prices[symbol] = final_prices[symbol][
                final_prices[symbol]["time"] <= end_time + resolution
            ]  # Add back
        ...
        for price_reader in self.__price_readers:
            ...
            for symbol in data:
                ...
                prices_by_resolution = aggregate_prices_by_resolution(
                    prices_by_resolution, symbol, resolution, data[symbol]
                )

                self.__check_user_time_bounds(start_time, stop_time, resolution)

        # Send the prices by resolution to the interface
        self.interface.receive_price_cache(
            sort_prices_by_resolution(prices_by_resolution)
        )
    ...
    def add_prices(
        ...
        if (
            (start_date is None and stop_date is not None)
            or (start_date is not None and stop_date is None)
            or (start is None and end is None)
        ):
            raise ValueError(
                "Both start and end dates must be set or use the 'to' argument."
            )
    ...
    def format_account_data(
        ...
        typing.Dict[typing.Union[str, typing.Any], typing.Union[int, typing.Any]],
        typing.Dict[typing.Union[str, typing.Any], typing.Union[int, typing.Any]],
    ]:
        ...
        try:
            quote_value = (
                true_account[self.quote_currency]["available"]
                + true_account[self.quote_currency]["hold"]
            )
        except KeyError as e:
            raise KeyError(
                f"Failed looking up {e}. Try changing your quote_account_value_in in user_data/backtest.json to be "
                f"{e}, or try a tether coin in user_data/backtest.json like USDT depending on exchange."
            )
        try:
        ...
        for i in list(true_account.keys()):
            ...
            no_trade_available[i] = (
                self.initial_account[i]["available"] + self.initial_account[i]["hold"]
            )
            currency_pair = i
            ...
            try:
                ...
                raise KeyError(
                    f"Failed to quote {currency_pair} because no downloaded data for that pair is available. "
                    f'Make sure to set "quote_account_value_in" in "user_data/backtest.json" to match the prices '
        ...
        no_trade_value += (
            self.initial_account[self.quote_currency]["available"]
    ...
    def __account_was_used(self, column) -> bool:
        ...
        is_used = (
            column in self.interface.traded_assets
            or "Account Value (" + self.quote_currency + ")" == column
        )
    ...
    def advance_time_and_price_index(self):
        ...
        for symbol in self.prices:
            ...
            self.interface.receive_price(
                symbol,
                new_price=self.prices[symbol][self.price_indexes[symbol]][
                    self.use_price
                ],
            )
    ...
    def sleep(self, seconds: [int, float]):
        ...
        if self.show_progress:
            if self.sleep_count % 300 == 0:
                ...
                update_progress(
                    (self.time - self.user_start) / (self.user_stop - self.user_start)
                )
    ...
    def value_account(self) -> None:
        ...
        available_dict, no_trade_dict = self.format_account_data(
            self.interface, self.time
        )
    ...
    def run(
        ...
        **kwargs,
        ...
        self.show_progress = self.preferences["settings"][
            "show_progress_during_backtest"
        ]

        if not exchange.get_type().endswith("paper_trade"):
            raise ValueError(
                "Backtest controller was not constructed with a paper trade exchange object."
            )
        ...
        if isinstance(self.interface, FuturesPaperTradeInterface):
            for symbol in self.prices:
                self.add_custom_events(
                    FundingRateEventReader(
                        symbol, self.user_start, self.user_stop, self.interface
                    )
                )
        ...
        if benchmark_symbol is not None:
            ...
            self.add_prices(
                benchmark_symbol,
                start_date=self.user_start,
                stop_date=self.user_stop,
        ...
        for frame_symbol, price_list in self.prices.items():
            ...
            try:
                ...
                if not check_if_any_column_has_prices(self.prices):
                    raise IndexError(
                        'No cached or downloaded data available. Try adding arguments such as to="1y" '
                    ...
                    raise IndexError(
                        f"Data for symbol {frame_symbol} is empty. Are you using a symbol that "
                        f"is incompatible "
        ...
        if self.preferences["settings"]["save_initial_account_value"]:
            available_dict, no_trade_dict = self.format_account_data(
                self.interface, self.user_start
            )
            self.traded_account_values.append(available_dict)
            self.no_trade_account_values.append(no_trade_dict)
        ...
        cycle_status = pd.concat(
            [cycle_status, pd.DataFrame(self.traded_account_values)], ignore_index=True
        ).sort_values(by=["time"])

        if len(cycle_status) == 0:
            raise RuntimeError(
                "Empty result - no valid backtesting events occurred. Was there an error?."
            )

        no_trade_cycle_status = pd.concat(
            [no_trade_cycle_status, pd.DataFrame(self.no_trade_account_values)],
            ignore_index=True,
        ).sort_values(by=["time"])
        ...
        history_and_returns: dict = {"history": cycle_status}
        metrics_indicators = {}
        user_callbacks = {}

        result_object = BacktestResult(
            history_and_returns,
            {
        ...
        resample_setting = self.preferences["settings"][
            "resample_account_value_for_metrics"
        ]
        ...
        resampled_account_data_frame = result_object.resample_account(
            "Account Value (" + self.quote_currency + ")", interval_value
        )
        ...
        # -----=====*****=====-----
        metrics_indicators["Compound Annual Growth Rate (%)"] = metrics.cagr(
            history_and_returns
        )
        try:
            metrics_indicators["Cumulative Returns (%)"] = metrics.cum_returns(
                history_and_returns
            )
        ...
        metrics_indicators["Max Drawdown (%)"] = attempt(
            metrics.max_drawdown, history_and_returns
        )
        metrics_indicators["Variance (%)"] = attempt(
            metrics.variance, history_and_returns, {"trading_period": interval_value}
        )
        metrics_indicators["Sortino Ratio"] = attempt(
            metrics.sortino,
            history_and_returns,
            {"risk_free_rate": risk_free_return_rate, "trading_period": interval_value},
        )
        metrics_indicators["Sharpe Ratio"] = attempt(
            metrics.sharpe,
            history_and_returns,
            {"risk_free_rate": risk_free_return_rate, "trading_period": interval_value},
        )
        metrics_indicators["Calmar Ratio"] = attempt(
            metrics.calmar, history_and_returns, {"trading_period": interval_value}
        )
        metrics_indicators["Volatility"] = attempt(
            metrics.volatility, history_and_returns, {"trading_period": interval_value}
        )
        metrics_indicators["Value-at-Risk"] = attempt(metrics.var, history_and_returns)
        metrics_indicators["Conditional Value-at-Risk"] = attempt(
            metrics.cvar, history_and_returns
        )
        ...
        if benchmark_symbol is not None:
            ...
            resampled_benchmark_value = result_object.resample_account(
                benchmark_symbol,
                interval_value,
            ...
            history_and_returns["benchmark_returns"] = resampled_benchmark_value.copy(
                deep=True
            )
            history_and_returns["benchmark_returns"]["value"] = history_and_returns[
                "benchmark_returns"
            ]["value"].pct_change()

            # Calculate beta
            metrics_indicators["Beta"] = attempt(
                metrics.beta, history_and_returns, {"trading_period": interval_value}
            )

        # Remove NaN values here
        history_and_returns["resampled_account_value"] = history_and_returns[
            "resampled_account_value"
        ].where(history_and_returns["resampled_account_value"].notnull(), None)

        # Remove NaN values on this one too
        history_and_returns["returns"] = history_and_returns["returns"].where(
            history_and_returns["returns"].notnull(), None
        )
        # Lastly remove Nan values in the metrics
        for symbol in metrics_indicators:
            if not isinstance(metrics_indicators[symbol], str) and np.isnan(
                metrics_indicators[symbol]
            ):
        ...
        if self.preferences["settings"]["gui_output"]:
            ...
            def internal_backtest_viewer():
                ...
                def add_trace(self_, figure_, time__, data_, label):
                    source = ColumnDataSource(
                        data=dict(time=time__, value=data_.values.tolist())
                    )
                    figure_.step(
                        "time",
                        "value",
                ...
                for column in cycle_status:
                    if column != "time" and self.__account_was_used(column):
                        p = figure(
                            frame_width=900, frame_height=200, x_axis_type="datetime"
                        )
                        ...
                        if column == "Account Value (" + self.quote_currency + ")":
                            add_trace(
                                ...
                                p,
                                time_,
                                no_trade_cycle_status["Account Value (No Trades)"],
                            ...
                            if benchmark_symbol is not None:
                                ...
                                initial_account_value = cycle_status[
                                    "Account Value (" + self.quote_currency + ")"
                                ].iloc[0]
                                benchmark_series = pd.Series(
                                    self.prices[benchmark_symbol][use_price]
                                )
                                ...
                                multiplier = (
                                    initial_account_value / initial_benchmark_value
                                )

                                normalized_compare_series = benchmark_series.multiply(
                                    multiplier
                                )
                                normalized_compare_time_series = self.prices[
                                    benchmark_symbol
                                ]["time"]
                                ...
                                normalized_compare_time_series = [
                                    dt.fromtimestamp(ts)
                                    for ts in normalized_compare_time_series
                                ]
                                add_trace(
                                    self,
                                    p,
                ...
                #            f'https://app.trapilot.finance/RETIe0J8EPSQz7wizoJX0OAFb8y1/62iIMVRKV7zkcpJysYlP/'
            ...
            # try:
            #     json_file = json.loads(open('./trapilot.json').read())
            ...
            #
            #     requests.post(f'https://events.trapilot.finance/v1/backtest/result', json=platform_result, headers={
            #         'api_key': api_key,
            #         'api_pass': api_pass,
            #         'model_id': model_id
            #     })
            #
            #     requests.post(f'https://events.trapilot.finance/v1/backtest/status', json={
            #         'successful': True,
            ...
            #     import webbrowser
            #
            #     link = f'https://app.trapilot.finance/{api_key}/{model_id}/{platform_result["backtest_id"]}' \
            #            f'/backtest'
            #     webbrowser.open(
            #         link
            #     )
        ...
        trapilot.blankly.reporter.export_backtest_result(platform_result)
