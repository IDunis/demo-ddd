...
import trapilot.blankly.exchanges.interfaces.paper_trade.utils as paper_trade
...
from trapilot.blankly.exchanges.interfaces.paper_trade.backtesting_wrapper import \
    BacktestingWrapper
from trapilot.blankly.exchanges.interfaces.paper_trade.local_account.trade_local import \
    LocalAccount
...
from trapilot.blankly.utils.exceptions import APIException, InvalidOrder


class PaperTradeInterface(ExchangeInterface, BacktestingWrapper):
    def __init__(
        self,
        derived_interface: ABCExchangeInterface,
        ...
        ExchangeInterface.__init__(
            self, derived_interface.get_exchange_type(), derived_interface
        )
        ...
        self.__enable_shorting = self.user_preferences["settings"]["alpaca"][
            "enable_shorting"
        ]
        ...
        if self.user_preferences["settings"]["paper"]["price_source"] == "websocket":
            from trapilot.blankly.exchanges.managers.ticker_manager import \
                TickerManager

            warnings.warn("Experimental websocket prices enabled.")
            self.__ticker_manager = TickerManager(
                self.get_exchange_type(), default_symbol=""
            )
                ...
                for i in accounts.keys():
                    if i in self.__initial_account_values.keys():
                        accounts[i] = utils.AttributeDict(
                            {"available": self.__initial_account_values[i], "hold": 0.0}
                        )
                    else:
                        accounts[i] = utils.AttributeDict(
    ...
    def evaluate_traded_account_assets(self):
        ...
        for i in accounts.keys():
            if (
                accounts[i]["available"] + accounts[i]["hold"]
            ) != 0 and i not in self.traded_assets:
    ...
    def init_exchange(self):
        try:
            ...
            raise AttributeError(
                "Are you passing a non-exchange object into the paper trade constructor?"
            )
    ...
    def start_paper_trade_watchdog(self):
        ...
        self.__thread = threading.Thread(
            target=self.__paper_trade_watchdog, daemon=True
        )
        self.__thread.start()
        self.__run_watchdog = True
        ...
        for k, v in current_account.items():
            if k in value_dictionary.keys():
                current_account[k] = utils.AttributeDict(
                    {"available": value_dictionary[k], "hold": 0}
                )
            else:
                current_account[k] = utils.AttributeDict({"available": 0, "hold": 0})
    ...
    def evaluate_limits(self):
        ...
        for i in used_currencies:
            ...
            decimals_dict[i] = {}
            decimals_dict[i]["quantity_decimals"] = self.__get_decimals(
                market_limits["limit_order"]["base_increment"]
            )
            decimals_dict[i]["quote_decimals"] = self.__get_decimals(
                market_limits["market_order"]["quote_increment"]
            )
            ...
            if index["type"] in ("limit", "stop_loss") and index["status"] == "pending":
                ...
                if index["side"] == "buy":
                    if index["price"] > current_price:
                        ...
                        self.local_account.update_available(
                            quote, available + (index["size"] * index["price"])
                        )

                        # Take it out of hold
                        hold = self.local_account.get_account(quote)["hold"]
                        self.local_account.update_hold(
                            quote, hold - (index["size"] * index["price"])
                        )

                        order, funds, executed_value, fill_fees, filled_size = (
                            self.evaluate_paper_trade(index, index["price"])
                        )
                        self.local_account.trade_local(
                            symbol=index["symbol"],
                            side="buy",
                    ...
                    if (
                        ...
                        and index["type"] == "limit"
                        or current_price <= limit_price
                        and index["type"] == "stop_loss"
                    ):
                        # Take everything off hold
    ...
    def evaluate_paper_trade(self, order, current_price):
        ...
        executed_value = funds - funds * float(
            (self.get_fees(order["symbol"])["maker_fee_rate"])
        )
        fill_fees = funds * float((self.get_fees(order["symbol"])["maker_fee_rate"]))
        fill_size = order["size"] - order["size"] * float(
            (self.get_fees(order["symbol"])["maker_fee_rate"])
        )
    ...
    def get_account(self, symbol=None) -> utils.AttributeDict:
        if symbol is None:
            ...
            try:
                ...
                if self.backtesting:
                    raise KeyError(
                        f"Symbol {symbol} not found. This can be caused by an invalid quote currency "
                        "in user_data/backtest.json."
                    )
                else:
    ...
    def take_profit_order(
        ...
        return TakeProfitOrder(
            order._LimitOrder__order, order._LimitOrder__response, order.Interface
        )
    ...
    def market_order(self, symbol, side, size) -> MarketOrder:
        ...
        if size < min_size:
            raise InvalidOrder(
                f"Size is too small. Minimum is: {min_size}. You requested {size}."
            )

        if size > max_size:
            raise InvalidOrder(
                f"Size is too large. Maximum is: {max_size}. You requested {size}."
            )
        ...
        if self.should_auto_trunc:
            ...
            raise InvalidOrder(
                "Size resolution is too high, the highest resolution allowed for this symbol is: "
                + str(base_increment)
                + ". You specified "
                + str(size)
                + ". Try using trapilot.blankly.trunc(size, decimal_number) to match the exchange resolution."
            )
        ...
        if self.get_exchange_type() == "alpaca":
            ...
            shortable = False
            quantity_decimals = self.__get_decimals(
                market_limits["limit_order"]["base_increment"]
            )
        ...
        if side == "buy":
            self.local_account.trade_local(
                symbol=symbol,
                side=side,
                base_delta=utils.trunc(
                    qty - qty * float((self.get_fees(symbol)["taker_fee_rate"])),
                    quantity_decimals,
                ),
                # Gain filled size after fees
                quote_delta=utils.trunc(
                    funds * -1, base_decimals
                ),  # Loose the original fund amount
                quote_resolution=base_decimals,
                base_resolution=quantity_decimals,
            )
        elif side == "sell":
            self.local_account.trade_local(
                symbol=symbol,
                ...
                # Loose size before any fees
                quote_delta=utils.trunc(
                    funds - funds * float((self.get_fees(symbol)["taker_fee_rate"])),
                    base_decimals,
        ...
        self.market_order_execution_details.append(
            {"id": response["id"], "executed_price": price}
        )
    ...
    def limit_order(self, symbol, side, price, size, stop_loss=False) -> LimitOrder:
        ...
        if size < min_base:
            raise InvalidOrder(
                "Order quantity is too small. Minimum is: " + str(min_base)
            )

        if size > max_base:
            raise InvalidOrder(
                "Order quantity is too large. Maximum is: " + str(max_base)
            )
        ...
        if price < min_price:
            raise InvalidOrder(
                "Limit price is too small. Minimum is: " + str(min_price)
            )

        if price > max_price:
            raise InvalidOrder(
                "Limit price is too large. Maximum is: " + str(max_price)
            )
        ...
        if self.should_auto_trunc:
            ...
            raise InvalidOrder(
                "Fund resolution is too high, minimum resolution is: "
                + str(price_increment)
                + ". Try using trapilot.blankly.trunc(size, decimal_number) to match the exchange resolution."
            )
        ...
        self.local_account.test_trade(
            ...
            size,
            price,
            quote_resolution=price_increment_decimals,
        ...
        return (
            StopLossOrder(order, response, self)
            if stop_loss
            else LimitOrder(order, response, self)
        )
    ...
    def cancel_order(self, symbol, order_id) -> dict:
        ...
        if order_index is not None:
            ...
            if side == "buy":
                ...
                self.local_account.update_available(
                    quote_asset, available + (size * price)
                )
    ...
    def get_products(self):
        def get_keyless_products():
            ...
            for full_symbol in self.full_prices:
                symbols_.append({"symbol": full_symbol})
    ...
    def get_product_history(self, symbol, epoch_start, epoch_stop, resolution):
        if self.backtesting:
            ...
            return self.calls.get_product_history(
    ...
    def get_order_filter(self, symbol):
        ...
        if self.get_exchange_type() == "binance":
            self.get_order_filter_cache[symbol] = self.__evaluate_binance_limits(
                self.get_price(symbol), self.get_order_filter_cache[symbol]
            )
    ...
    def get_price(self, symbol) -> float:
        if self.backtesting:
            ...
            if (
                self.user_preferences["settings"]["paper"]["price_source"]
                == "websocket"
            ):
                tickers = self.__ticker_manager.get_all_tickers()
                if (
                    self.get_exchange_type() in tickers
                    and symbol in tickers[self.get_exchange_type()]
                ):
                    most_recent_tick = tickers[self.get_exchange_type()][
                        symbol
                    ].get_most_recent_tick()
                    ...
                    utils.info_print(
                        f"Creating ticker on symbol {symbol} for exchange {self.get_exchange_type()}..."
                    )
                    self.__ticker_manager.create_ticker(
                        callback=self._websocket_update,
    ...
    def __evaluate_binance_limits(price: (int, float), order_filter):
        order_filter["limit_order"]["min_price"] = (
            order_filter["exchange_specific"]["limit_multiplier_down"] * price
        )
        order_filter["limit_order"]["max_price"] = (
            order_filter["exchange_specific"]["limit_multiplier_up"] * price
        )
        return order_filter
