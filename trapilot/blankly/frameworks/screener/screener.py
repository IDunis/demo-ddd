...
from trapilot.blankly.frameworks.screener.screener_runner import ScreenerRunner
from trapilot.blankly.frameworks.screener.screener_state import ScreenerState
from trapilot.blankly.utils.utils import load_deployment_settings
...
class Screener:
    def __init__(
        ...
        Function Signatures:
        init(screener_state: trapilot.blankly.ScreenerState)
        evaluator(symbol: str, screener_state: trapilot.blankly.ScreenerState)
        final(ScreenerState: trapilot.blankly.ScreenerState)
        formatter(raw_results: dict, screener_state: trapilot.blankly.ScreenerState)
        ...
        if trapilot.blankly.is_deployed:
            trapilot.blankly.reporter.export_screener(self)
    ...
    def __run(self):
        ...
        trapilot.blankly.reporter.export_screener_result(self)

    def notify(self, message: str = None):
        """
        Send an email and text message to yourself. When deployed live this will come from an official trapilot email &
         phone number. When run locally it will use information found in your user_data/notify.json file.
        ...
        trapilot.blankly.reporter.email(use_str)

        trapilot.blankly.reporter.text(use_str)
