...
class StrategyBase:
    ...
    def __init__(
        ...
        Create a new strategy object. A strategy can be used to run your code live while be backtestable and modular
         across exchanges.
         Args:
             exchange: An exchange object. This can be created by doing something similar to exchange = trapilot.blankly.Alpaca()

        Function Signatures:
        init(symbol: str, state: trapilot.blankly.StrategyState)
        price_event(price: float, symbol: str, state: trapilot.blankly.StrategyState)
        orderbook_event(tick: dict, symbol: str, state: trapilot.blankly.StrategyState)
        bar_event(bar: dict, symbol: str, state: trapilot.blankly.StrategyState)
        teardown(trapilot.StrategyState)
        """
        self.remote_backtesting = trapilot.blankly._backtesting
        ...
        trapilot.blankly.reporter.export_strategy(self)
    ...
    def __custom_price_event(
        ...
        if symbol is not None:
            if isinstance(symbol, list):
                for i in symbol:
                    trapilot.blankly.reporter.export_used_symbol(i)
            else:
                trapilot.blankly.reporter.export_used_symbol(symbol)
