...
import trapilot
from trapilot.blankly.data import PriceReader


def price_event(price, symbol, state: trapilot.blankly.StrategyState):
    """This function will give an updated price every 15 seconds from our definition below"""
    state.variables["history"].append(price)
    rsi = trapilot.blankly.indicators.rsi(state.variables["history"])
    curr_value = state.interface.account[state.base_asset].available
    if rsi[-1] < 30 and not curr_value:
        # Dollar cost average buy
        buy = trapilot.blankly.trunc(state.interface.cash / price, 5)
...
def init(symbol, state: trapilot.blankly.StrategyState):
...
if __name__ == "__main__":
    ...
    exchange = trapilot.blankly.KeylessExchange(
