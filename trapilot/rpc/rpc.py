...
from trapilot.enums import (CandleType, ExitCheckTuple, ExitType, MarketDirection, SignalDirection,
                             State, TradingMode)
...
from trapilot.util import (decimals_per_coin, dt_humanize, dt_now, dt_ts_def, format_date,
                            shorten_date)
...
class RPC:
    ...
    def __balance_get_est_stake(
        ...
        if coin == stake_currency:
            ...
            pair = self._tradebot.exchange.get_valid_pair_combination(coin, stake_currency)
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        ...
        if order_side == SignalDirection.SHORT and self._tradebot.trading_mode == TradingMode.SPOT:
    ...
    def _rpc_force_entry(self, pair: str, price: Optional[float], *,
        ...
        with self._tradebot._exit_lock:
            if self._tradebot.execute_entry(pair, stake_amount, price,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._tradebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._tradebot.exchange.fetch_order(open_order.order_id, trade.pair)
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._tradebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._tradebot.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass

            # cancel stoploss on exchange orders ...
            if (self._tradebot.strategy.order_types.get('stoploss_on_exchange')
                    and trade.has_open_sl_orders):

                for oslo in trade.open_sl_orders:
                    try:
                        self._tradebot.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except (ExchangeError):
                        pass
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            'current': len(trades),
            'max': (int(self._tradebot.config['max_open_trades'])
                    if self._tradebot.config['max_open_trades'] != float('inf') else -1),
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {'method': self._tradebot.pairlists.name_list,
        ...
        if add:
            for pair in add:
                if pair not in self._tradebot.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._tradebot.exchange.get_markets().keys())
    ...
    def _rpc_analysed_dataframe(self, pair: str, timeframe: str,
        ...
        return RPC._convert_dataframe_to_dict(self._tradebot.config['strategy'],
    ...
    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (self._tradebot.strategy.plot_config and
                'subplots' not in self._tradebot.strategy.plot_config):
