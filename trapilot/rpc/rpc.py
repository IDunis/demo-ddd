...
from trapilot.loggers import bufferHandler
from trapilot.persistence import KeyStoreKeys, KeyValueStore, PairLocks, Trade
...
class RPC:
    ...
    def __init__(self, trapilot) -> None:
        """
        Initializes all enabled rpc modules
        :param trapilot: Instance of a trapilot bot
        :return: None
        """
        self._tradebot = trapilot
        self._config: Config = trapilot.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "trapilot"),
            "timeframe": config.get("timeframe"),
            "timeframe_ms": (
                timeframe_to_msecs(config["timeframe"]) if "timeframe" in config else 0
            ),
            "timeframe_min": (
            ...
            "position_adjustment_enable": config.get(
    ...
    def _rpc_trade_status(self, trade_ids: List[int] = []) -> List[Dict[str, Any]]:
        ...
        # Fetch open trades
        if trade_ids:
            trades: Sequence[Trade] = Trade.get_trades(
                trade_filter=Trade.id.in_(trade_ids)
            ).all()
        else:
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._tradebot.wallets.get_total_stake_amount()
        ...
        for day in range(0, timescale):
            ...
            profit_units[profitday] = {
                ...
                "rel_profit": (
                    round(curdayprofit / daily_stake, 8) if daily_stake > 0 else 0
                ),
                "trades": len(trades),
            }
    ...
    def _rpc_trade_history(
        self, limit: int, offset: int = 0, order_by_id: bool = False
    ) -> Dict:
        ...
        if limit:
            ...
            trades = Trade.session.scalars(
                Trade.get_trades_query([Trade.is_open.is_(False)]).order_by(
    ...
    def _rpc_stats(self) -> Dict[str, Any]:
        ...
        draws_dur = (
            sum(dur["draws"]) / len(dur["draws"]) if len(dur["draws"]) > 0 else None
        )
        losses_dur = (
            sum(dur["losses"]) / len(dur["losses"]) if len(dur["losses"]) > 0 else None
        )
    ...
    def _rpc_trade_statistics(
        ...
        trades: Sequence[Trade] = Trade.session.scalars(
            Trade.get_trades_query(trade_filter, include_orders=False).order_by(
                Trade.id
            )
        ).all()
        ...
        profit_closed_ratio_sum = (
            sum(profit_closed_ratio) if profit_closed_ratio else 0.0
        )
        ...
        starting_balance = self._tradebot.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == stake_currency:
            ...
            pair = self._tradebot.exchange.get_valid_pair_combination(
                coin, stake_currency
            )
            rate: Optional[float] = tickers.get(pair, {}).get("last", None)
            if rate:
                if pair.startswith(stake_currency) and not pair.endswith(
                    stake_currency
                ):
                    rate = 1.0 / rate
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total_bot = 0.0
        try:
            tickers: Tickers = self._tradebot.exchange.get_tickers(cached=True)
        except ExchangeError:
            raise RPCException("Error getting current tickers.")

        open_trades: List[Trade] = Trade.get_open_trades()
        open_assets: Dict[str, Trade] = {t.safe_base_currency: t for t in open_trades}
        self._tradebot.wallets.update(require_update=False)
        starting_capital = self._tradebot.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._tradebot.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                trade_amount = self._tradebot.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._tradebot.wallets.get_all_positions().items():
        ...
        value = (
            self._fiat_converter.convert_amount(
                total, stake_currency, fiat_display_currency
            )
            if self._fiat_converter
            else 0
        )
        value_bot = (
            self._fiat_converter.convert_amount(
                total_bot, stake_currency, fiat_display_currency
            )
            if self._fiat_converter
            else 0
        )
        ...
        starting_capital_ratio = (
            (total_bot / starting_capital) - 1 if starting_capital else 0.0
        )
        starting_cap_fiat_ratio = (
            (value_bot / starting_cap_fiat) - 1 if starting_cap_fiat else 0.0
        )
        ...
        return {
            ...
            "note": "Simulated balances" if self._tradebot.config["dry_run"] else "",
        }
    ...
    def _rpc_stopentry(self) -> Dict[str, str]:
        ...
        if self._tradebot.state == State.RUNNING:
            ...
            self._tradebot.config["max_open_trades"] = 0
            self._tradebot.strategy.max_open_trades = 0
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> Dict[str, str]:
        ...
        self._tradebot.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            trade_entry_cancelation_res = {
                "order_id": oo.order_id,
                "cancel_state": False,
            }
            order = self._tradebot.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._tradebot.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._tradebot.handle_cancel_exit(
                    trade, order, oo, CANCEL_REASON["FORCE_EXIT"]
                )

        if all(
            tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry
        ):
            ...
            order_type = ordertype or self._tradebot.strategy.order_types.get(
                "force_exit", self._tradebot.strategy.order_types["exit"]
            )
            sub_amount: Optional[float] = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._tradebot.exchange.get_min_pair_stake_amount(
            ...
            self._tradebot.execute_trade_exit(
                trade,
                current_rate,
                exit_check,
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._tradebot.config.get("force_entry_enable", False):
        ...
        if (
            ...
            and self._tradebot.trading_mode == TradingMode.SPOT
        ):
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._tradebot.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._tradebot.config.get("stake_currency")
        if not self._tradebot.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            is_short = trade.is_short
            if not self._tradebot.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._tradebot.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._tradebot.strategy.order_types.get(
                "force_entry", self._tradebot.strategy.order_types["entry"]
            )
        with self._tradebot._exit_lock:
            if self._tradebot.execute_entry(
                pair,
                stake_amount,
                price,
                ordertype=order_type,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._tradebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._tradebot.exchange.fetch_order(
                        open_order.order_id, trade.pair
                    )
                except ExchangeError as e:
                    logger.info(
                        f"Cannot query order for {trade} due to {e}.", exc_info=True
                    )
                    raise RPCException("Order not found.")
                self._tradebot.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._tradebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._tradebot.exchange.cancel_order(
                        open_order.order_id, trade.pair
                    )
                    c_count += 1
                except ExchangeError:
            ...
            if (
                self._tradebot.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):

                for oslo in trade.open_sl_orders:
                    try:
                        self._tradebot.exchange.cancel_stoploss_order(
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._tradebot.config["max_open_trades"])
                if self._tradebot.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_delete_lock(
        ...
        if lockid:
            locks = PairLock.session.scalars(
                select(PairLock).filter(PairLock.id == lockid)
            ).all()
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {
            "method": self._tradebot.pairlists.name_list,
            "length": len(self._tradebot.active_pair_whitelist),
            "whitelist": self._tradebot.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: List[str]) -> Dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._tradebot.pairlists.blacklist:
                self._tradebot.pairlists.blacklist.remove(pair)
            else:
                errors[pair] = {
    ...
    def _rpc_blacklist(self, add: Optional[List[str]] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._tradebot.pairlists.blacklist:
                    try:
                        expand_pairlist(
                            [pair], self._tradebot.exchange.get_markets().keys()
                        )
                        self._tradebot.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._tradebot.pairlists.name_list,
            "length": len(self._tradebot.pairlists.blacklist),
            "blacklist": self._tradebot.pairlists.blacklist,
            "blacklist_expanded": self._tradebot.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        #       "trapilot.worker", "INFO", "Starting worker develop"]

        return {"log_count": len(records), "logs": records}

    def _rpc_edge(self) -> List[Dict[str, Any]]:
        """Returns information related to Edge"""
        if not self._tradebot.edge:
            raise RPCException("Edge is not enabled.")
        return self._tradebot.edge.accepted_pairs()
    ...
    def _convert_dataframe_to_dict(
        ...
        if has_content:
            ...
            dataframe.loc[:, "__date_ts"] = (
                dataframe.loc[:, "date"].astype(int64) // 1000 // 1000
            )
            ...
            for sig_type in signals.keys():
                if sig_type in dataframe.columns:
                    ...
                    dataframe.loc[mask, f"_{sig_type}_signal_close"] = dataframe.loc[
                        mask, "close"
                    ]
            ...
            for date_column in date_columns:
                ...
                dataframe[date_column] = (
                    dataframe[date_column].astype(object).replace({NaT: None})
                )
    ...
    def _rpc_analysed_dataframe(
        ...
        return RPC._convert_dataframe_to_dict(
            self._tradebot.config["strategy"], pair, timeframe, _data, last_analyzed
        )
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._tradebot.dataprovider.get_analyzed_dataframe(
            pair, timeframe
        )
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._tradebot.config["timeframe"]
        candle_type = self._tradebot.config.get("candle_type_def", CandleType.SPOT)

        for pair in pairlist:
            dataframe, last_analyzed = self.__rpc_analysed_dataframe_raw(
    ...
    def _ws_request_analyzed_df(
        self, limit: Optional[int] = None, pair: Optional[str] = None
    ):
        """Historical Analyzed Dataframes for WebSocket"""
        pairlist = [pair] if pair else self._tradebot.active_pair_whitelist

        return self._ws_all_analysed_dataframes(pairlist, limit)

    def _ws_request_whitelist(self):
        """Whitelist data for WebSocket"""
        return self._tradebot.active_pair_whitelist

    @staticmethod
    def _rpc_analysed_history_full(
        config: Config, pair: str, timeframe: str, exchange
    ) -> Dict[str, Any]:
        ...
        return RPC._convert_dataframe_to_dict(
            strategy.get_strategy_name(), pair, timeframe, df_analyzed.copy(), dt_now()
        )

    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (
            self._tradebot.strategy.plot_config
            and "subplots" not in self._tradebot.strategy.plot_config
        ):
            self._tradebot.strategy.plot_config["subplots"] = {}
        return self._tradebot.strategy.plot_config
    ...
    def health(self) -> Dict[str, Optional[Union[str, int]]]:
        last_p = self._tradebot.last_process
        if last_p is None:
            return {
                "last_process": None,
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._tradebot.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._tradebot.strategy.market_direction
