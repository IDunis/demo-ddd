...
class Telegram(RPCHandler):
    ...
    def _init_keyboard(self) -> None:
        ...
        if cust_keyboard:
            ...
            invalid_keys = [
                b
                for b in chain.from_iterable(cust_keyboard)
            ...
            if len(invalid_keys):
                ...
                logger.info(
                    "using custom keyboard from " f"config.json: {self._keyboard}"
                )
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = (
                    self._rpc._tradebot.dataprovider.get_analyzed_dataframe(
                        pair, self._config["timeframe"]
                    )
                )
                candle = (
                    analyzed_df.iloc[-1].squeeze() if len(analyzed_df) > 0 else None
                )
    ...
    def _format_entry_msg(self, msg: RPCEntryMsg) -> str:
        ...
        message += (
            f"*{'New ' if msg['sub_trade'] else ''}Total:* `{total}{profit_fiat_extra}`"
        )
    ...
    def compose_message(self, msg: RPCSendMsg) -> Optional[str]:
        if (
            msg["type"] == RPCMessageType.ENTRY
            or msg["type"] == RPCMessageType.ENTRY_FILL
        ):
            message = self._format_entry_msg(msg)

        elif (
            msg["type"] == RPCMessageType.EXIT
            or msg["type"] == RPCMessageType.EXIT_FILL
        ):
            ...
            message_side = (
                "enter" if msg["type"] == RPCMessageType.ENTRY_CANCEL else "exit"
            )
            message = (
    ...
    def _prepare_order_details(
        self, filled_orders: List, quote_currency: str, is_open: bool
    ):
        """
        ...
        for order in filled_orders:
            ...
            if order_nr == 1:
                ...
                lines.append(
                    f"*Average {wording} Price:* {cur_entry_average:.8g} "
        ...
        for line in lines:
            if line:
                if (len(msg) + len(line) + 1) < MAX_MESSAGE_LENGTH:
                    ...
                    msg = (
                        "*Order List for Trade #*`{trade_id}` - continued\n"
                        + line
                        + "\n"
                    )
    ...
    async def _tracking_prices(self, update: Update, context: CallbackContext) -> None:
        try:
            ...
            for symbol in tickers:
                ...
                await self._send_msg(f"*{symbol}:* `{ticker['last']}`")
        except Exception:
    ...
    async def _status_table(self, update: Update, context: CallbackContext) -> None:
        ...
        for i in range(0, messages_count):
            ...
            if show_total and i == messages_count - 1:
                # append total line
                trades.append(
                    ["Total", "", "", f"{fiat_profit_sum:.2f} {fiat_currency}"]
                )
    ...
    async def _timeunit_stats(
        self, update: Update, context: CallbackContext, unit: str
    ) -> None:
        """
        Handler for /daily <n>
        ...
        vals = {
            "days": TimeunitMappings(
                "Day", "Daily", "days", "update_daily", 7, "%Y-%m-%d"
            ),
            ...
            "months": TimeunitMappings(
                "Month", "Monthly", "months", "update_monthly", 6, "%Y-%m"
            ),
        }
        val = vals[unit]
        ...
        stats = self._rpc._rpc_timeunit_profit(
            timescale, stake_cur, fiat_disp_cur, unit
        )
        stats_tab = tabulate(
            [
                [
    ...
    async def _profit(self, update: Update, context: CallbackContext) -> None:
        ...
        first_trade_date = (
            f"{stats['first_trade_humanized']} ({stats['first_trade_date']})"
        )
        latest_trade_date = (
            f"{stats['latest_trade_humanized']} ({stats['latest_trade_date']})"
        )
    ...
    async def _stats(self, update: Update, context: CallbackContext) -> None:
        ...
        for reason in chunks(exit_reasons_tabulate, 25):
            exit_reasons_msg = tabulate(
                reason, headers=["Exit Reason", "Exits", "Wins", "Losses"]
            )
        ...
        for curr in result["currencies"]:
            ...
            if (curr["is_position"] or curr["est_stake"] > balance_dust_level) and (
                ...
                if curr["is_position"]:
                    ...
                    est_stake = fmt_coin(
                        curr["est_stake" if full_result else "est_stake_bot"],
                        curr["stake"],
                        False,
                    )
    ...
    @authorized_only
    async def _reload_trade_from_exchange(
        ...
        if context.args:
            ...
            trade_buttons = [
                InlineKeyboardButton(
                    text=trade[1], callback_data=f"force_exit__{trade[0]}"
                )
                for trade in trades
            ]
        ...
        if trade_id != "cancel":
            try:
                ...
                await loop.run_in_executor(
                    None, safe_async_db(self._rpc._rpc_force_exit), trade_id
                )
            except RPCException as e:
                await self._send_msg(str(e))
    ...
    async def _force_enter_action(
        self, pair, price: Optional[float], order_side: SignalDirection
    ):
        if pair != "cancel":
            try:
        ...
        if update.callback_query:
            ...
            if query.data and "__" in query.data:
                ...
                if payload and "_||_" in payload:
                    ...
                    await query.answer()
                    await query.edit_message_text(
                        text=f"Manually entering {order_side} for {pair}"
                    )
    ...
    async def _force_enter(
        ...
        if context.args:
            ...
            buttons_aligned.append(
                [
                    InlineKeyboardButton(
                        text="Cancel", callback_data="force_enter__cancel"
                    )
                ]
            )
            await self._send_msg(
    ...
    @authorized_only
    async def _enter_tag_performance(
        self, update: Update, context: CallbackContext
    ) -> None:
        """
    ...
    @authorized_only
    async def _exit_reason_performance(
        self, update: Update, context: CallbackContext
    ) -> None:
        """
        Handler for /exits.
    ...
    @authorized_only
    async def _mix_tag_performance(
        self, update: Update, context: CallbackContext
    ) -> None:
        """
    ...
    async def _whitelist(self, update: Update, context: CallbackContext) -> None:
        ...
        if context.args:
            ...
            if "baseonly" in context.args:
                whitelist["whitelist"] = [
                    pair.split("/")[0] for pair in whitelist["whitelist"]
                ]
    ...
    async def _blacklist_delete(self, update: Update, context: CallbackContext) -> None:
        ...
        """
        await self.send_blacklist_msg(
            self._rpc._rpc_blacklist_delete(context.args or [])
        )
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._tradebot.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._tradebot.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._tradebot.state)
