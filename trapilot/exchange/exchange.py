...
from trapilot.constants import (DEFAULT_AMOUNT_RESERVE_PERCENT,
                                NON_OPEN_EXCHANGE_STATES, BidAsk, BuySell,
                                Config, EntryExit, ExchangeConfig,
                                ListPairsWithTimeframes, MakerTaker, OBLiteral,
                                PairWithTimeframe)
from trapilot.data.converter import (clean_ohlcv_dataframe, ohlcv_to_dataframe,
                                     trades_dict_to_list)
from trapilot.enums import (OPTIMIZE_MODES, CandleType, MarginMode, PriceType,
                            RunMode, TradingMode)
from trapilot.exceptions import (DDosProtection, ExchangeError,
                                 InsufficientFundsError, InvalidOrderException,
                                 OperationalException, PricingError,
                                 RetryableOrderError, TemporaryError)
from trapilot.exchange.common import (API_FETCH_ORDER_RETRY_COUNT,
                                      remove_exchange_credentials, retrier,
                                      retrier_async)
from trapilot.exchange.exchange_utils import (ROUND, ROUND_DOWN, ROUND_UP,
                                              CcxtModuleType,
...
from trapilot.exchange.types import OHLCVResponse, OrderBook, Ticker, Tickers
from trapilot.misc import (chunks, deep_merge_dicts, file_dump_json,
                           file_load_json, safe_value_fallback2)
from trapilot.plugins.pairlist.pairlist_helpers import expand_pairlist
from trapilot.util import dt_from_ts, dt_now
from trapilot.util.datetime_helpers import dt_humanize, dt_ts
...
class Exchange:
    ...
    def __init__(
        ...
        *,
        ...
        exchange_conf: Dict[str, Any] = (
            exchange_config if exchange_config else config["exchange"]
        )
        ...
        if exchange_conf.get("_ft_has_params"):
            self._ft_has = deep_merge_dicts(
                exchange_conf.get("_ft_has_params"), self._ft_has
            )
            logger.info(
        ...
        ccxt_config = deep_merge_dicts(
            exchange_conf.get("ccxt_config", {}), ccxt_config
        )
        ccxt_config = deep_merge_dicts(
            exchange_conf.get("ccxt_sync_config", {}), ccxt_config
        )
    ...
    def _init_ccxt(
        ...
        try:
            ...
            raise OperationalException(
                f"Initialization of ccxt failed. Reason: {e}"
            ) from e
    ...
    def get_markets(
        ...
        if quote_currencies:
            markets = {
                k: v for k, v in markets.items() if v["quote"] in quote_currencies
            }
        if tradable_only:
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Trapilot.
        ...
        return (
            ...
            and (
                ...
                or (
                    self.trading_mode == TradingMode.MARGIN
                    and self.market_is_margin(market)
                )
                or (
                    self.trading_mode == TradingMode.FUTURES
                    and self.market_is_future(market)
                )
            )
        )
    ...
    def klines(self, pair_interval: PairWithTimeframe, copy: bool = True) -> DataFrame:
        if pair_interval in self._klines:
            return (
                ...
                if copy
                else self._klines[pair_interval]
            )
        else:
            return DataFrame()
    ...
    def validate_pairs(self, pairs: List[str]) -> None:
        ...
        for pair in extended_pairs:
            ...
            elif isinstance(self.markets[pair].get("info"), dict) and self.markets[
                pair
            ].get("info", {}).get("prohibitedIn", False):
    ...
    def validate_timeframes(self, timeframe: Optional[str]) -> None:
        ...
        if (
            ...
            raise OperationalException(
                "Timeframes < 1m are currently not supported by Trapilot."
            )
    ...
    def validate_pricing(self, pricing: Dict) -> None:
        if pricing.get("use_order_book", False) and not self.exchange_has(
            "fetchL2OrderBook"
        ):
    ...
    def validate_required_startup_candles(
        ...
        required_candle_call_count = int(
            (candle_count / candle_limit)
            + (0 if candle_count % candle_limit == 0 else 1)
        )
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        margin_mode: Optional[
            MarginMode
        ],  # Only None when trading_mode = TradingMode.SPOT
    ):
        """
        Checks if trapilot can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by trapilot on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"Trapilot does not support {mm_value} {trading_mode.value} on {self.name}"
            )
    ...
    def price_to_precision(
        self, pair: str, price: float, *, rounding_mode: int = ROUND
    ) -> float:
        """
    ...
    def get_max_pair_stake_amount(
        ...
        max_stake_amount = self._get_stake_amount_limit(
            pair, price, 0.0, "max", leverage
        )
        if max_stake_amount is None:
            # * Should never be executed
            raise OperationalException(
                f"{self.name}.get_max_pair_stake_amount should"
    ...
    def _get_stake_amount_limit(
        ...
        if limits["amount"][limit] is not None:
            stake_limits.append(
                self._contracts_to_amount(pair, limits["amount"][limit])
                * price
                * margin_reserve
            )
    ...
    def create_dry_run_order(
        ...
        _amount = self._contracts_to_amount(
            pair,
            self.amount_to_precision(pair, self._amount_to_contracts(pair, amount)),
        )
        ...
        if dry_order["type"] == "market" and not dry_order.get("ft_order_type"):
            ...
            average = self.get_dry_market_fill_price(
                pair, side, amount, rate, orderbook
            )
            dry_order.update(
                {
                    "average": average,
    ...
    def get_dry_market_fill_price(
        ...
        if self.exchange_has("fetchL2OrderBook"):
            ...
            if side == "buy":
                forecast_avg_filled_price = min(
                    forecast_avg_filled_price, max_slippage_val
                )

            else:
                forecast_avg_filled_price = max(
                    forecast_avg_filled_price, max_slippage_val
                )
    ...
    def check_dry_limit_order_filled(
        ...
        if (
            ...
            if self._dry_is_price_crossed(
                pair, order["side"], order["price"], orderbook
            ):
                order.update(
                    {
                        "status": "closed",
    ...
    def fetch_dry_run_order(self, order_id) -> Dict[str, Any]:
        ...
        try:
            ...
            from trapilot.persistence import Order
    ...
    def _get_stop_limit_rate(
        self, stop_price: float, order_types: Dict, side: str
    ) -> float:
    ...
    def create_stoploss(
        ...
        stop_price_norm = self.price_to_precision(
            pair, stop_price, rounding_mode=round_mode
        )
        limit_rate = None
        if user_order_type == "limit":
            limit_rate = self._get_stop_limit_rate(stop_price, order_types, side)
            limit_rate = self.price_to_precision(
                pair, limit_rate, rounding_mode=round_mode
            )
    ...
    def cancel_order(self, order_id: str, pair: str, params: Dict = {}) -> Dict:
        if self._config["dry_run"]:
            try:
                ...
                order.update(
                    {"status": "canceled", "filled": 0.0, "remaining": order["amount"]}
                )
                return order
    ...
    def cancel_stoploss_order(
        self, order_id: str, pair: str, params: Dict = {}
    ) -> Dict:
        ...
        try:
            ...
            if self.exchange_has("fetchOrders"):
                if not params:
                    params = {}
                try:
                    orders: List[Dict] = self._api.fetch_orders(
    ...
    def fetch_ticker(self, pair: str) -> Ticker:
        try:
            if (
                pair not in self.markets
                or self.markets[pair].get("active", False) is False
            ):
    ...
    def get_rates(
        ...
        if not entry_rate and entry_pricing.get("use_order_book", False):
            order_book_top = max(
                entry_pricing.get("order_book_top", 1),
                exit_pricing.get("order_book_top", 1),
            )
            order_book = self.fetch_l2_order_book(pair, order_book_top)
            entry_rate = self.get_rate(
                pair, refresh, "entry", is_short, order_book=order_book
            )
        elif not entry_rate:
            ticker = self.fetch_ticker(pair)
    ...
    def calculate_fee_rate(
        ...
        if fee_curr == self.get_pair_base_currency(symbol):
            ...
            if not cost:
                ...
                return None
            try:
                comb = self.get_valid_pair_combination(
                ...
                fee_to_quote_rate = self._config["exchange"].get(
    ...
    async def _async_get_historic_ohlcv(
        ...
        logger.debug(
            ...
            one_call,
            dt_humanize(
                dt_now() - timedelta(milliseconds=one_call), only_distance=True
            ),
        )
        input_coroutines = [
    ...
    def _build_coroutine(
        ...
        if cache and (pair, timeframe, candle_type) in self._klines:
            ...
            if min_date < self._pairs_last_refresh_time.get(
    ...
    async def _async_get_candle_history(
        ...
        try:
            ...
            logger.debug(
                "Done fetching pair %s, %s interval %s...", pair, candle_type, timeframe
            )
            return pair, timeframe, candle_type, data, self._ohlcv_partial_candle
        ...
        try:
            ...
            if params:
                ...
                logger.debug(
                    ...
                    since,
                    (
                        "(" + dt_from_ts(since).isoformat() + ") "
                        if since is not None
                        else ""
                    ),
                )
                trades = await self._api_async.fetch_trades(
                    pair, since=since, limit=1000
                )
    ...
    async def _async_get_trade_history(
        ...
        if self._trades_pagination == "time":
            ...
            raise OperationalException(
                f"Exchange {self.name} does use neither time, "
    ...
    def get_historic_trades(
        ...
        if not self.exchange_has("fetchTrades"):
            raise OperationalException(
                "This exchange does not support downloading Trades."
            )

        with self._loop_lock:
            task = asyncio.ensure_future(
                self._async_get_trade_history(
                    pair=pair, since=since, until=until, from_id=from_id
                )
            )
    ...
    def _get_funding_fees_from_exchange(
        self, pair: str, since: Union[datetime, int]
    ) -> float:
        """
    ...
    def cache_leverage_tiers(
        self, tiers: Dict[str, List[Dict]], stake_currency: str
    ) -> None:

        filename = (
            self._config["datadir"]
            / "futures"
            / f"leverage_tiers_{stake_currency}.json"
        )
    ...
    def get_max_leverage(self, pair: str, stake_amount: Optional[float]) -> float:
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            if stake_amount == 0:
                return self._leverage_tiers[pair][0][
                    "maxLeverage"
                ]  # Max lev for lowest amount
            ...
            for tier_index in range(len(pair_tiers)):
                ...
                if tier_index < len(pair_tiers) - 1:
                    ...
                    if (
                        next_floor > stake_amount
                    ):  # Next tier min too high for stake amount
                    ...
                    if stake_amount > tier["maxNotional"]:
                        ...
                        raise InvalidOrderException(
                            f"Amount {stake_amount} too high for {pair}"
                        )
                    else:
            ...
            self.trading_mode == TradingMode.MARGIN
        ):  # Search markets.limits for max lev
            market = self.markets[pair]
    ...
    def combine_funding_and_mark(
        ...
        futures_funding_rate: Optional[int] = None,
    ) -> DataFrame:
        """
        ...
        if futures_funding_rate is None:
            ...
            if len(funding_rates) == 0:
                ...
                combined["open_fund"] = combined["open_fund"].fillna(
    ...
    def get_liquidation_price(
        ...
        if liquidation_price is not None:
            ...
            liquidation_price_buffer = (
                ...
                if is_short
                else liquidation_price + buffer_amount
            )
    ...
    def dry_run_liquidation_price(
        ...
        if (
            self.trading_mode == TradingMode.FUTURES
            and self.margin_mode == MarginMode.ISOLATED
        ):

            if market["inverse"]:
                raise OperationalException(
                    "Trapilot does not yet support inverse contracts"
                )
