...
from trapilot.exchange import available_exchanges, is_exchange_known_ccxt, validate_exchange
from trapilot.exchange.common import MAP_EXCHANGE_CHILDCLASS, SUPPORTED_EXCHANGES
...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    """
    Check if the exchange name in the config file is supported by Trapilot
    :param check_for_bad: if True, check the exchange against the list of known 'bad'
                          exchanges
    :return: False if exchange is 'bad', i.e. is known to work with the bot with
             critical issues or does not work at all, crashes, etc. True otherwise.
             raises an exception if the exchange if not supported by ccxt
             and thus is not known for the Trapilot at all.
    """
    ...
    if not exchange:
        raise OperationalException(
            ...
            f'The following exchanges are available for Trapilot: '
    ...
    if not valid:
        if check_for_bad:
            raise OperationalException(f'Exchange "{exchange}"  will not work with Trapilot. '
                                       f'Reason: {reason}')
        else:
            logger.warning(f'Exchange "{exchange}"  will not work with Trapilot. Reason: {reason}')

    if MAP_EXCHANGE_CHILDCLASS.get(exchange, exchange) in SUPPORTED_EXCHANGES:
        logger.info(f'Exchange "{exchange}" is officially supported '
                    f'by the Trapilot development team.')
    else:
