...
from trapilot.exchange import (available_exchanges, is_exchange_known_ccxt,
                               validate_exchange)
from trapilot.exchange.common import (MAP_EXCHANGE_CHILDCLASS,
                                      SUPPORTED_EXCHANGES)
...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    ...
    if not valid:
        if check_for_bad:
            raise OperationalException(
                f'Exchange "{exchange}"  will not work with Trapilot. '
                f"Reason: {reason}"
            )
        else:
            logger.warning(
                f'Exchange "{exchange}"  will not work with Trapilot. Reason: {reason}'
            )
