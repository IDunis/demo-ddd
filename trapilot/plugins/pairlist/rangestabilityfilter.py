...
from trapilot.misc import plural
from trapilot.plugins.pairlist.IPairList import IPairList, PairlistParameter
from trapilot.util import dt_floor_day, dt_now, dt_ts
...
class RangeStabilityFilter(IPairList):
    ...
    def __init__(
        ...
        candle_limit = exchange.ohlcv_candle_limit(
            "1d", self._config["candle_type_def"]
        )
        if self._days < 1:
            raise OperationalException(
                "RangeStabilityFilter requires lookback_days to be >= 1"
            )
    ...
    def filter_pairlist(self, pairlist: List[str], tickers: Tickers) -> List[str]:
        ...
        candles = self._exchange.refresh_ohlcv_with_cache(
            needed_pairs, since_ms=since_ms
        )
    ...
    def _calculate_rate_of_change(
        self, pair: str, daily_candles: DataFrame
    ) -> Optional[float]:
        ...
        if daily_candles is not None and not daily_candles.empty:
            ...
            pct_change = (
                ((highest_high - lowest_low) / lowest_low) if lowest_low > 0 else 0
            )
