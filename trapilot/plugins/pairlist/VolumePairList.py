...
from trapilot.constants import Config, ListPairsWithTimeframes
from trapilot.exceptions import OperationalException
from trapilot.exchange import timeframe_to_minutes, timeframe_to_prev_date
from trapilot.exchange.types import Tickers
from trapilot.plugins.pairlist.IPairList import IPairList, PairlistParameter
from trapilot.util import dt_now, format_ms_time
...
class VolumePairList(IPairList):
    ...
    def __init__(
        ...
        self._sort_key: Literal["quoteVolume"] = self._pairlistconfig.get(
        ...
        if pairlist:
            ...
            if not self._use_range:
                filtered_tickers = [
                    ...
                    if (
                        self._exchange.get_pair_quote_currency(k)
    ...
    def filter_pairlist(self, pairlist: List[str], tickers: Dict) -> List[str]:
        ...
        if self._use_range:
            ...
            since_ms = (
                int(
                    timeframe_to_prev_date(
                        ...
                        dt_now()
                        + timedelta(
                            minutes=-(self._lookback_period * self._tf_in_min)
                            - self._tf_in_min
                        ),
                    ).timestamp()
                )
            ...
            for i, p in enumerate(filtered_tickers):
                contract_size = (
                    self._exchange.markets[p["symbol"]].get("contractSize", 1.0) or 1.0
                )
                pair_candles = (
                    candles[
                        (p["symbol"], self._lookback_timeframe, self._def_candletype)
                    ]
                    if (p["symbol"], self._lookback_timeframe, self._def_candletype)
                    in candles
                    else None
                )
        ...
        sorted_tickers = sorted(
            filtered_tickers, reverse=True, key=lambda t: t[self._sort_key]
        )

        # Validate whitelist to only have active market pairs
        pairs = self._whitelist_for_active_markets(
            [s["symbol"] for s in sorted_tickers]
        )
