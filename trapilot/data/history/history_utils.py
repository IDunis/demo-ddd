...
from trapilot.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist
from trapilot.util import dt_ts, format_ms_time
    ...
    *,
    ...
    for pair in pairs:
        ...
        if not hist.empty:
            ...
            if (
                candle_type is CandleType.FUNDING_RATE
    ...
    try:
        ...
        new_data = exchange.get_historic_ohlcv(
            ...
            since_ms=(
                since_ms
                if since_ms
                else int((datetime.now() - timedelta(days=new_pairs_days)).timestamp())
                * 1000
            ),
...
def refresh_backtest_ohlcv_data(
    ...
    Used by trapilot download-data subcommand.
    ...
    for idx, pair in enumerate(pairs, start=1):
        ...
        if trading_mode == "futures":
            ...
            combs = (
                (CandleType.FUNDING_RATE, tf_funding_rate),
    ...
    try:
        ...
        logger.debug(
            ...
            (
                "None"
                if trades.empty
                else f"{trades.iloc[0]['date']:{DATETIME_PRINT_FORMAT}}"
            ),
        )
        logger.debug(
            "Current End: %s",
            (
                "None"
                if trades.empty
                else f"{trades.iloc[-1]['date']:{DATETIME_PRINT_FORMAT}}"
            ),
        )
...
def download_data_main(config: Config) -> None:
    ...
    timerange = TimeRange()
    if "days" in config:
        time_since = (datetime.now() - timedelta(days=config["days"])).strftime(
            "%Y%m%d"
        )
