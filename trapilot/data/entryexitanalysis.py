...
from trapilot.data.btanalysis import (get_latest_backtest_filename,
                                      load_backtest_data, load_backtest_stats)
    ...
    try:
        ...
        for pair in pairlist:
            if pair in signal_candles[strategy_name]:
                analysed_trades_dict[strategy_name][pair] = (
...
def _analyze_candles_and_indicators(
    pair, trades: pd.DataFrame, signal_candles: pd.DataFrame
):
    buyf = signal_candles
    ...
    if len(buyf) > 0:
        ...
        if trades_red.shape[0] > 0 and buyf.shape[0] > 0:
            ...
            if "signal_date" in trades_red:
                ...
                try:
                    trades_red = pd.merge(
                        trades_red, trades_inds, on="signal_date", how="outer"
                    )
                except Exception as e:
                    raise e
        return trades_red
...
def prepare_results(
    ...
    if (
        res_df is not None
        and res_df.shape[0] > 0
        and ("enter_reason" in res_df.columns)
    ):
...
def _print_table(
    ...
    *,
    ...
    if to_csv:
        safe_name = Path(
            csv_path, name.lower().replace(" ", "_").replace(":", "") + ".csv"
        )
