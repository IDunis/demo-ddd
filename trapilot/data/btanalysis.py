...
from trapilot.constants import LAST_BT_RESULT_FN, IntOrInf
from trapilot.exceptions import OperationalException
from trapilot.misc import file_dump_json, json_load
from trapilot.optimize.backtest_caching import get_backtest_metadata_filename
from trapilot.persistence import LocalTrade, Trade, init_db
from trapilot.types import BacktestHistoryEntryType, BacktestResultType
...
def load_and_merge_backtest_result(
    strategy_name: str, filename: Path, results: Dict[str, Any]
):
    """
...
def load_backtest_data(
    filename: Union[Path, str], strategy: Optional[str] = None
) -> pd.DataFrame:
    """
...
def analyze_trade_parallelism(results: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    """
    from trapilot.exchange import timeframe_to_resample_freq

    timeframe_freq = timeframe_to_resample_freq(timeframe)
    dates = [
        pd.Series(
            pd.date_range(
                row[1]["open_date"], row[1]["close_date"], freq=timeframe_freq
            )
        )
    ...
    df2 = pd.DataFrame(
        np.repeat(results.values, deltas, axis=0), columns=results.columns
    )
...
def trade_list_to_dataframe(
    trades: Union[List[Trade], List[LocalTrade]]
) -> pd.DataFrame:
    """
    ...
    df = pd.DataFrame.from_records(
        [t.to_json(True) for t in trades], columns=BT_DATA_COLUMNS
    )
    if len(df) > 0:
        df["close_date"] = pd.to_datetime(df["close_date"], utc=True)
        df["open_date"] = pd.to_datetime(df["open_date"], utc=True)
