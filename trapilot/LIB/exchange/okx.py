...
from trapilot.LIB.util import dt_now, dt_ts
...
class Okx(Exchange):
    ...
    def ohlcv_candle_limit(
        ...
        * 100 candles for additional candles (not futures or spot).
        ...
        if candle_type in (CandleType.FUTURES, CandleType.SPOT) and (
            not since_ms
            or since_ms > (date_minus_candles(timeframe, 300).timestamp() * 1000)
        ):
            return 300
    ...
    def additional_exchange_init(self) -> None:
        ...
        try:
            if self.trading_mode == TradingMode.FUTURES and not self._config["dry_run"]:
                ...
                if len(accounts) > 0:
                    self.net_only = (
                        accounts[0].get("info", {}).get("posMode") == "net_mode"
                    )
    ...
    def get_max_pair_stake_amount(
        ...
        if self.trading_mode == TradingMode.SPOT:
            return float(
                "inf"
            )  # Not actually inf, but this probably won't matter for SPOT
    ...
    def _fetch_orders_emulate(self, pair: str, since_ms: int) -> List[Dict]:
        ...
        if since_ms < dt_ts(dt_now() - timedelta(days=6, hours=23)):
            ...
            orders_hist = self._api.fetch_closed_orders(
