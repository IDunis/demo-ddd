...
from trapilot.LIB.util import render_template, render_template_with_fallback
...
def deploy_new_strategy(
    strategy_name: str, strategy_path: Path, subtemplate: str
) -> None:
    """
...
def start_new_strategy(args: Dict[str, Any]) -> None:
    ...
    if "strategy" in args and args["strategy"]:
        ...
        new_path = (
            config["user_data_dir"] / USERPATH_STRATEGIES / (args["strategy"] + ".py")
        )
...
def clean_ui_subdir(directory: Path):
    if directory.is_dir():
        ...
        for p in reversed(
            list(directory.glob("**/*"))
        ):  # iterate contents from leaves to root
...
def get_ui_download_url(version: Optional[str] = None) -> Tuple[str, str]:
    base_url = "https://api.github.com/repos/trapilot/frequi/"
