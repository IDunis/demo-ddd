...
from trapilot.LIB.commands.cli_options import AVAILABLE_CLI_OPTIONS
from trapilot.LIB.constants import DEFAULT_CONFIG
...
ARGS_CONVERT_DATA_OHLCV = ARGS_CONVERT_DATA + [
    "timeframes",
    "trading_mode",
    "candle_types",
]
...
ARGS_LOOKAHEAD_ANALYSIS = [
    ...
    if a not in ("position_stacking", "use_max_market_positions", "cache")
] + [
...
class Arguments:
    ...
    def _parse_args(self) -> argparse.Namespace:
        ...
        if "config" in parsed_arg and parsed_arg.config is None:
            conf_required = (
                "command" in parsed_arg and parsed_arg.command in NO_CONF_REQURIED
            )
    ...
    def _build_subcommands(self) -> None:
        ...
        from trapilot.LIB.commands import (start_analysis_entries_exits,
                                           start_backtesting,
                                           ...
                                           start_hyperopt_show,
                                           start_install_ui, start_list_data,
        ...
        convert_trade_data_cmd.set_defaults(
            func=partial(start_convert_data, ohlcv=False)
        )
        self._build_args(
            optionlist=ARGS_CONVERT_DATA_TRADES, parser=convert_trade_data_cmd
        )
        ...
        list_markets_cmd.set_defaults(
            func=partial(start_list_markets, pairs_only=False)
        )
        ...
        self._build_args(
            optionlist=ARGS_LIST_FREQAIMODELS, parser=list_freqaimodels_cmd
        )
        ...
        self._build_args(
            optionlist=ARGS_LOOKAHEAD_ANALYSIS, parser=lookahead_analayis_cmd
        )
        ...
        self._build_args(
            optionlist=ARGS_RECURSIVE_ANALYSIS, parser=recursive_analayis_cmd
        )
