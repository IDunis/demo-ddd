...
from trapilot.LIB.exchange import list_available_exchanges, market_is_active
from trapilot.LIB.misc import parse_db_uri_for_logging, plural
...
def start_list_exchanges(args: Dict[str, Any]) -> None:
    ...
    if args["print_one_column"]:
        ...
        def build_entry(exchange: ValidExchangesType, valid: bool):
            ...
            result: Dict[str, Union[str, bool]] = {
                ...
                **valid_entry,
                ...
                "trade_modes": ", ".join(
                    (f"{a['margin_mode']} " if a["margin_mode"] else "")
                    + a["trading_mode"]
        ...
        if args["list_exchanges_all"]:
            ...
            print("Exchanges available for Trapilot:")
            exchanges = [
                build_entry(e, False) for e in exchanges if e["valid"] is not False
            ]
...
def start_list_freqAI_models(args: Dict[str, Any]) -> None:
    ...
    model_objs = FreqaiModelResolver.search_all_objects(
        config, not args["print_one_column"]
    )
    # Sort alphabetically
    ...
    try:
        ...
        if pairs:
            if args.get("print_list", False):
                ...
                print(
                    tabulate(
                        tabular_data, headers="keys", tablefmt="psql", stralign="right"
                    )
                )
        elif not (
            args.get("print_one_column", False)
...
def start_show_trades(args: Dict[str, Any]) -> None:
    ...
    from trapilot.LIB.persistence import Trade, init_db
