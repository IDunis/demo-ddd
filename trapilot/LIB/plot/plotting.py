...
from trapilot.LIB.data.btanalysis import (analyze_trade_parallelism,
                                          extract_trades_of_period,
                                          load_trades)
from trapilot.LIB.data.converter import trim_dataframe
from trapilot.LIB.data.dataprovider import DataProvider
from trapilot.LIB.data.history import get_timerange, load_data
from trapilot.LIB.data.metrics import (calculate_max_drawdown,
...
from trapilot.LIB.exchange import timeframe_to_prev_date, timeframe_to_seconds
from trapilot.LIB.misc import pair_to_filename
...
def add_max_drawdown(
    ...
    try:
        ...
        drawdown = go.Scatter(
            ...
            marker=dict(
                symbol="square-open", size=9, line=dict(width=2), color="green"
            ),
        )
...
def add_underwater(
    fig, row, trades: pd.DataFrame, starting_balance: float
) -> make_subplots:
    """
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="Trapilot Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
    ...
    fig["layout"]["yaxis6"].update(
        title="Underwater Plot Relative (%)", tickformat=",.2%"
    )
    fig["layout"]["xaxis"]["rangeslider"].update(visible=False)
    fig.update_layout(modebar_add=["v1hovermode", "toggleSpikeLines"])
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "trapilot-plot-" + pair_s + "-" + timeframe + ".html"
...
def store_plot_file(
    fig, filename: str, directory: Path, auto_open: bool = False
) -> None:
    """
...
def load_and_plot_trades(config: Config):
    ...
    strategy_safe_wrapper(strategy.bot_loop_start)(
        current_time=datetime.now(timezone.utc)
    )
    plot_elements = init_plotscript(
        config, list(exchange.markets), strategy.startup_candle_count
    )
    ...
    for pair, data in plot_elements["ohlcv"].items():
        ...
        fig = generate_candlestick_graph(
            ...
            plot_config=(
                strategy.plot_config if hasattr(strategy, "plot_config") else {}
            ),
        )
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="trapilot-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
