...
from trapilot.LIB.constants import SUPPORTED_FIAT
from trapilot.LIB.mixins.logging_mixin import LoggingMixin
...
class CryptoToFiatConverter(LoggingMixin):
    ...
    def _get_gekko_id(self, crypto_symbol):
        ...
        if crypto_symbol in coingecko_mapping.keys():
            found = [
                x
                for x in self._coinlistings
                if x["id"] == coingecko_mapping[crypto_symbol]
            ]
    ...
    def convert_amount(
        self, crypto_amount: float, crypto_symbol: str, fiat_symbol: str
    ) -> float:
        """
    ...
    def _find_price(self, crypto_symbol: str, fiat_symbol: str) -> float:
        ...
        try:
            return float(
                self._coingekko.get_price(ids=_gekko_id, vs_currencies=fiat_symbol)[
                    _gekko_id
                ][fiat_symbol]
            )
        except Exception as exception:
