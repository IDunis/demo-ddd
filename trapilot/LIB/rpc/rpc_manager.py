...
from trapilot.LIB.enums import NO_ECHO_MESSAGES, RPCMessageType
from trapilot.LIB.rpc import RPC, RPCHandler
...
class RPCManager:
    ...
    def __init__(self, tradebot) -> None:
        ...
        if config.get("telegram", {}).get("enabled", False):
            ...
            from trapilot.LIB.rpc.telegram import Telegram
        ...
        if config.get("discord", {}).get("enabled", False):
            ...
            from trapilot.LIB.rpc.discord import Discord
        ...
        if config.get("webhook", {}).get("enabled", False):
            ...
            from trapilot.LIB.rpc.webhook import Webhook
    ...
    def send_msg(self, msg: RPCSendMsg) -> None:
        ...
        for mod in self.registered_modules:
            ...
            try:
                ...
                logger.error(
                    f"Message type '{msg['type']}' not implemented by handler {mod.name}."
                )
            except Exception:
    ...
    def startup_messages(self, config: Config, pairlist, protections) -> None:
        ...
        if len(protections.name_list) > 0:
            prots = "\n".join(
                [p for prot in protections.short_desc() for k, p in prot.items()]
            )
            self.send_msg(
                {
                    "type": RPCMessageType.STARTUP,
