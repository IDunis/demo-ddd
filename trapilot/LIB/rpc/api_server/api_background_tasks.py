...
from trapilot.LIB.persistence import FtNoDBContext
from trapilot.LIB.rpc.api_server.api_schemas import (BackgroundTaskStatus,
                                                     BgJobStarted,
...
@router.get(
    "/background/{jobid}", response_model=BackgroundTaskStatus, tags=["webserver"]
)
...
@router.get(
    "/pairlists/available",
    response_model=PairListsResponse,
    tags=["pairlists", "webserver"],
)
def list_pairlists(config=Depends(get_config)):
    from trapilot.LIB.resolvers import PairListResolver
...
@router.post(
    "/pairlists/evaluate", response_model=BgJobStarted, tags=["pairlists", "webserver"]
)
def pairlists_evaluate(
    ...
    if ApiBG.pairlist_running:
        raise HTTPException(
            status_code=400, detail="Pairlist evaluation is already running."
        )
