...
from trapilot.LIB.rpc.api_server.api_schemas import AccessAndRefreshToken, AccessToken
from trapilot.LIB.rpc.api_server.deps import get_api_config
...
def http_basic_or_jwt_token(
    ...
    if token:
        return get_user_from_token(
            token, api_config.get("jwt_secret_key", "super-secret")
        )
...
def token_refresh(
    token: str = Depends(oauth2_scheme), api_config=Depends(get_api_config)
):
    # Refresh token
