...
from trapilot.LIB.data.history import get_datahandler
from trapilot.LIB.enums import CandleType, TradingMode
from trapilot.LIB.exceptions import OperationalException
from trapilot.LIB.rpc import RPC
from trapilot.LIB.rpc.api_server.api_schemas import (AvailablePairs, Balances,
                                                     BlacklistPayload,
                                                     BlacklistResponse, Count,
                                                     DailyWeeklyMonthly,
                                                     DeleteLockRequest,
                                                     DeleteTrade, Entry,
                                                     ExchangeListResponse,
                                                     Exit, ForceEnterPayload,
                                                     ForceEnterResponse,
                                                     ForceExitPayload,
                                                     FreqAIModelListResponse,
                                                     Health, Locks, Logs,
                                                     MixTag, OpenTradeSchema,
                                                     PairHistory,
                                                     PerformanceEntry, Ping,
                                                     PlotConfig, Profit,
                                                     ResultMsg, ShowConfig,
                                                     Stats, StatusMsg,
                                                     StrategyListResponse,
                                                     StrategyResponse, SysInfo,
                                                     Version,
                                                     WhitelistResponse)
from trapilot.LIB.rpc.api_server.deps import (get_config, get_exchange,
                                              get_rpc, get_rpc_optional)
...
def monthly(
    timescale: int = 3, rpc: RPC = Depends(get_rpc), config=Depends(get_config)
):
    return rpc._rpc_timeunit_profit(
        timescale,
        config["stake_currency"],
...
@router.get(
    "/trade/{tradeid}", response_model=OpenTradeSchema, tags=["info", "trading"]
)
...
@router.delete(
    "/trades/{tradeid}", response_model=DeleteTrade, tags=["info", "trading"]
)
...
@router.delete(
    "/trades/{tradeid}/open-order", response_model=OpenTradeSchema, tags=["trading"]
)
...
@router.post(
    "/trades/{tradeid}/reload", response_model=OpenTradeSchema, tags=["trading"]
)
...
def show_config(
    rpc: Optional[RPC] = Depends(get_rpc_optional), config=Depends(get_config)
):
    state = ""
    strategy_version = None
    if rpc:
        state = rpc._tradebot.state
        strategy_version = rpc._tradebot.strategy.version()
...
def blacklist_delete(
    pairs_to_delete: List[str] = Query([]), rpc: RPC = Depends(get_rpc)
):
...
def get_strategy(strategy: str, config=Depends(get_config)):
    if ":" in strategy:
        raise HTTPException(
            status_code=500, detail="base64 encoded strategies are not allowed."
        )
...
def list_exchanges(config=Depends(get_config)):
    from trapilot.LIB.exchange import list_available_exchanges
