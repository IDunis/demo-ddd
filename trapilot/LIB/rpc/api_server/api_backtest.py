...
from trapilot.LIB.data.btanalysis import (delete_backtest_result,
                                          get_backtest_result,
...
from trapilot.LIB.rpc.api_server.api_schemas import (BacktestHistoryEntry,
...
def __run_backtest_bg(btconfig: Config):
    ...
    try:
        ...
        if (
            ApiBG.bt["bt"].results
            and strategy_name in ApiBG.bt["bt"].results["strategy"]
        ):
        ...
        if btconfig.get("export", "none") == "trades":
            ...
            ApiBG.bt["bt"].results["metadata"][strategy_name][
                "strategy"
            ] = strategy_name
...
@router.post(
    "/backtest", response_model=BacktestResponse, tags=["webserver", "backtest"]
)
...
@router.get(
    "/backtest/abort", response_model=BacktestResponse, tags=["webserver", "backtest"]
)
...
@router.get(
    "/backtest/history/result",
    response_model=BacktestResponse,
    tags=["webserver", "backtest"],
)
def api_backtest_history_result(
    filename: str, strategy: str, config=Depends(get_config)
):
    # Get backtest result history, read from metadata files
