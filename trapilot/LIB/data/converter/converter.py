...
from trapilot.LIB.constants import DEFAULT_DATAFRAME_COLUMNS, Config
...
def ohlcv_fill_up_missing_data(
    ...
    """
    from trapilot.LIB.exchange import timeframe_to_resample_freq
...
def convert_ohlcv_format(
    ...
    paircombs.extend(
        src.ohlcv_get_available_data(config["datadir"], TradingMode.FUTURES)
    )
    ...
    for pair, timeframe, candle_type in paircombs:
        ...
        if len(data) > 0:
            trg.ohlcv_store(
                pair=pair, timeframe=timeframe, data=data, candle_type=candle_type
            )
...
def reduce_dataframe_footprint(df: DataFrame) -> DataFrame:
    ...
    logger.debug(
        f"Memory usage of dataframe is " f"{df.memory_usage().sum() / 1024**2:.2f} MB"
    )
    ...
    logger.debug(
        f"Memory usage after optimization is: "
