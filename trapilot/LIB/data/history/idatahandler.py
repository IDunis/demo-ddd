...
from trapilot.LIB.constants import DEFAULT_TRADES_COLUMNS, ListPairsWithTimeframes
...
class IDataHandler(ABC):
    ...
    @classmethod
    def ohlcv_get_pairs(
        cls, datadir: Path, timeframe: str, candle_type: CandleType
    ) -> List[str]:
        """
        Returns a list of all pairs with ohlcv data available in this datadir
    ...
    def ohlcv_data_min_max(
        ...
        return (
            df.iloc[0]["date"].to_pydatetime(),
            df.iloc[-1]["date"].to_pydatetime(),
            len(df),
        )
    ...
    def trades_load(
        ...
        trades = trades_df_remove_duplicates(
            self._trades_load(pair, timerange=timerange)
        )
        ...
        *,
        ...
        if self._check_empty_df(pairdf, pair, timeframe, candle_type, warn_no_data):
            ...
            if timerange_startup:
                self._validate_pairdata(
                    pair, pairdf, timeframe, candle_type, timerange_startup
                )
            ...
            pairdf = clean_ohlcv_dataframe(
                ...
                drop_incomplete=(
                    drop_incomplete and enddate == pairdf.iloc[-1]["date"]
                ),
            )
        ...
        if pairdf.empty:
            if warn_no_data:
                logger.warning(
                    ...
                    "Use `trapilot download-data` to download the data"
                )
            return True
        elif warn_price:
            candle_price_gap = 0
            if (
                ...
                gaps = (pairdf["open"] - pairdf["close"].shift(1)) / pairdf[
                    "close"
                ].shift(1)
                gaps = gaps.dropna()
                if len(gaps):
    ...
    def rename_futures_data(
        ...
        file_new = self._pair_data_filename(
            self._datadir, new_pair, timeframe, candle_type
        )
    ...
    def fix_funding_fee_timeframe(self, ff_timeframe: str):
        ...
        funding_rate_combs = [
            f
            for f in paircombs
            if f[2] == CandleType.FUNDING_RATE and f[1] != ff_timeframe
        ]
        ...
        for pair, timeframe, candletype in funding_rate_combs:
            old_name = self._pair_data_filename(
                self._datadir, pair, timeframe, candletype
            )
            new_name = self._pair_data_filename(
                self._datadir, pair, ff_timeframe, candletype
            )
