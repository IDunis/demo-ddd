...
from trapilot.LIB.strategy.strategy_helper import merge_informative_pair
    ...
    * {timeframe} - timeframe of informative dataframe.
    ...
    def decorator(fn: PopulateIndicators):
        ...
        informative_pairs.append(
            InformativeData(_asset, _timeframe, _fmt, _ffill, _candle_type)
        )
...
def _format_pair_name(
    config, pair: str, market: Optional[Dict[str, Any]] = None
) -> str:
    return pair.format(
...
def _create_and_merge_informative_pair(
    ...
    inf_dataframe.rename(
        columns=lambda column: formatter(column=column, **fmt_args), inplace=True
    )
