...
from trapilot.LIB.loggers.set_log_levels import (
    reduce_verbosity_for_bias_tester,
    restore_verbosity_for_bias_tester,
)
from trapilot.LIB.optimize.backtesting import Backtesting
from trapilot.LIB.optimize.base_analysis import BaseAnalysis, VarHolder
...
class LookaheadAnalysis(BaseAnalysis):
    ...
    def analyze_indicators(
        self, full_vars: VarHolder, cut_vars: VarHolder, current_pair: str
    ):
        # extract dataframes
        ...
        full_df_cut = full_df[(full_df.date == cut_vars.compared_dt)].reset_index(
            drop=True
        )
        cut_df_cut = cut_df[(cut_df.date == cut_vars.compared_dt)].reset_index(
            drop=True
        )
            ...
            if compare_df.shape[0] > 0:
                for col_name, values in compare_df.items():
                    ...
                    if self_value != other_value:
                        ...
                        if not self.current_analysis.false_indicators.__contains__(
                            col_name[0]
                        ):
    ...
    def prepare_data(self, varholder: VarHolder, pairs_to_load: List[DataFrame]):
        ...
        if (
            "freqai" in self.local_config
            and "identifier" in self.local_config["freqai"]
        ):
        ...
        varholder.indicators = backtesting.strategy.advise_all_indicators(
            varholder.data
        )
    ...
    def analyze_row(self, idx: int, result_row):
        ...
        self.analyze_indicators(
            self.full_varHolder, self.entry_varHolders[idx], result_row["pair"]
        )
        self.analyze_indicators(
            self.full_varHolder, self.exit_varHolders[idx], result_row["pair"]
        )
    ...
    def start(self) -> None:
        ...
        if found_signals >= self.targeted_trade_amount:
            ...
            logger.info(
                f"Only found {found_signals} trades. Calculating all available trades."
            )
        else:
            logger.info(
                f"found {found_signals} trades "
        ...
        for idx, result_row in self.full_varHolder.result["results"].iterrows():
            if self.current_analysis.total_signals == self.targeted_trade_amount:
                logger.info(
                    f"Found targeted trade amount = {self.targeted_trade_amount} signals."
                )
                break
        ...
        if (
            self.current_analysis.total_signals
            < self.local_config["minimum_trade_amount"]
        ):
            logger.info(
