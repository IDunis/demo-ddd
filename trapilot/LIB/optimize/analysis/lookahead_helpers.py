...
from trapilot.LIB.optimize.analysis.lookahead import LookaheadAnalysis
...
class LookaheadAnalysisSubFunctions:
    ...
    @staticmethod
    def export_to_csv(
        config: Dict[str, Any], lookahead_analysis: List[LookaheadAnalysis]
    ):
        ...
        for inst in lookahead_analysis:
            ...
            if (
                ...
                new_row_data = {
                    ...
                    "biased_indicators": ",".join(
                        inst.current_analysis.false_indicators
                    ),
                }
        ...
        csv_df["biased_entry_signals"] = (
            csv_df["biased_entry_signals"].astype(int).fillna(0)
        )
        csv_df["biased_exit_signals"] = (
            csv_df["biased_exit_signals"].astype(int).fillna(0)
        )
    ...
    def initialize_single_lookahead_analysis(
        config: Config, strategy_obj: Dict[str, Any]
    ):
    ...
    def start(config: Config):
        ...
        if lookaheadAnalysis_instances:
            ...
            if config.get("lookahead_analysis_exportfilename") is not None:
                LookaheadAnalysisSubFunctions.export_to_csv(
                    config, lookaheadAnalysis_instances
                )
        else:
            logger.error(
