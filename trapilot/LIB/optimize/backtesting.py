...
from trapilot.LIB.configuration import TimeRange, validate_config_consistency
from trapilot.LIB.constants import (DATETIME_PRINT_FORMAT, Config, IntOrInf,
                                    LongShort)
from trapilot.LIB.data import history
from trapilot.LIB.data.btanalysis import (find_existing_backtest_stats,
                                          trade_list_to_dataframe)
from trapilot.LIB.data.converter import trim_dataframe, trim_dataframes
from trapilot.LIB.data.dataprovider import DataProvider
from trapilot.LIB.enums import (BacktestState, CandleType, ExitCheckTuple,
                                ExitType, RunMode, TradingMode)
from trapilot.LIB.exceptions import DependencyException, OperationalException
from trapilot.LIB.exchange import (amount_to_contract_precision,
                                   price_to_precision, timeframe_to_seconds)
from trapilot.LIB.exchange.exchange import Exchange
from trapilot.LIB.mixins import LoggingMixin
from trapilot.LIB.optimize.backtest_caching import get_strategy_run_id
from trapilot.LIB.optimize.bt_progress import BTProgress
from trapilot.LIB.optimize.optimize_reports import (
...
from trapilot.LIB.persistence import (LocalTrade, Order, PairLocks, Trade,
                                      disable_database_use,
                                      enable_database_use)
from trapilot.LIB.plugins.pairlistmanager import PairListManager
from trapilot.LIB.plugins.protectionmanager import ProtectionManager
from trapilot.LIB.resolvers import ExchangeResolver, StrategyResolver
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Optional[Exchange] = None) -> None:
        ...
        if self.config.get("freqai", {}).get("enabled", False):
            ...
            self.required_startup = self.dataprovider.get_required_startup(
                self.timeframe
            )
    ...
    def load_bt_data_detail(self) -> None:
        ...
        if self.trading_mode == TradingMode.FUTURES:
            self.funding_fee_timeframe: str = self.exchange.get_option(
                "funding_fee_timeframe"
            )
            self.funding_fee_timeframe_secs: int = timeframe_to_seconds(
                self.funding_fee_timeframe
            )
            ...
            mark_rates_dict = history.load_data(
                ...
                candle_type=CandleType.from_string(
                    self.exchange.get_option("mark_ohlcv_price")
                ),
            )
    ...
    def _get_ohlcv_as_lists(self, processed: Dict[str, DataFrame]) -> Dict[str, Tuple]:
        ...
        for pair in processed.keys():
            ...
            data[pair] = (
                df_analyzed[HEADERS].values.tolist() if not df_analyzed.empty else []
            )
        return data
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            close_rate = -(roi_rate + open_fee_rate) / (
                (trade.fee_close or 0.0) - side_1 * 1
            )
            if is_short:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/trapilot/trapilot/issues/6261
    ...
    def _get_adjust_trade_entry_for_candle(
        ...
        if stake_amount is not None and stake_amount > 0.0:
            ...
            if self.strategy.max_entry_position_adjustment > -1:
                ...
                check_adjust_entry = (
                    entry_count <= self.strategy.max_entry_position_adjustment
                )
        ...
        if stake_amount is not None and stake_amount < 0.0:
            amount = amount_to_contract_precision(
                abs(stake_amount * trade.leverage) / current_rate,
            ...
            pos_trade = self._get_exit_for_signal(
                trade, row, exit_, current_time, amount
            )
            if pos_trade is not None:
    ...
    def _call_adjust_stop(
        self, current_date: datetime, trade: LocalTrade, current_rate: float
    ):
    ...
    def _try_close_open_order(
        ...
        if order and self._get_order_filled(order.ft_price, row):
            ...
            if not (
                ...
                and order.safe_amount == trade.amount
            ):
                # trade is still open
    ...
    def _get_exit_for_signal(
        ...
        if exit_.exit_flag:
            ...
            trade_dur = int(
                (trade.close_date_utc - trade.open_date_utc).total_seconds() // 60
            )
            try:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # trapilot does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def _run_funding_fees(
        self, trade: LocalTrade, current_time: datetime, force: bool = False
    ):
        """
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which trapilot does not support in live.
            if new_rate is not None and new_rate != propose_rate:
                propose_rate = price_to_precision(
                    new_rate, price_precision, self.precision_mode
                )
            if direction == "short":
    ...
    def _enter_trade(
        ...
        entry_tag = entry_tag1 or (
            row[ENTER_TAG_IDX] if len(row) >= ENTER_TAG_IDX + 1 else None
        )
    ...
    def handle_left_open(
        ...
        for pair in open_trades.keys():
            for trade in list(open_trades[pair]):
                ...
                trade.orders[-1].close_bt_order(
                    exit_row[DATE_IDX].to_pydatetime(), trade
                )
    ...
    def manage_open_orders(
        self, trade: LocalTrade, current_time: datetime, row: Tuple
    ) -> bool:
        """
        ...
        for order in [o for o in trade.orders if o.ft_is_open]:
            ...
            if oc:
                ...
                return True
            elif oc is None and self.check_order_replace(
                trade, order, current_time, row
            ):
    ...
    def backtest(
        ...
        return {
            ...
            "final_balance": self.wallets.get_total(
                self.strategy.config["stake_currency"]
            ),
        }
    ...
    def backtest_one_strategy(
        ...
        preprocessed_tmp = trim_dataframes(
            preprocessed, timerange, self.required_startup
        )
    ...
    def _get_min_cached_backtest_date(self):
        ...
        backtest_cache_age = self.config.get(
            "backtest_cache", constants.BACKTEST_CACHE_DEFAULT
        )
        if self.timerange.stopts == 0 or self.timerange.stopdt > datetime.now(
            tz=timezone.utc
        ):
            logger.warning(
                "Backtest result caching disabled due to use of open-ended timerange."
            )
    ...
    def load_prior_backtest(self):
        ...
        if min_backtest_date is not None:
            self.results = find_existing_backtest_stats(
                self.config["user_data_dir"] / "backtest_results",
                self.run_ids,
                min_backtest_date,
            )
        ...
        if len(self.all_results) > 0:
            ...
            if self.config.get("export", "none") in ("trades", "signals"):
                store_backtest_stats(
                    self.config["exportfilename"], self.results, dt_appendix
                )
