...
from trapilot.LIB.constants import UNLIMITED_STAKE_AMOUNT, Config
from trapilot.LIB.optimize.optimize_reports.optimize_reports import \
...
def _get_line_floatfmt(stake_currency: str) -> List[str]:
    ...
    return [
        ...
        ".2f",
        ".2f",
        f".{decimals_per_coin(stake_currency)}f",
        ".2f",
        "d",
        "s",
        "s",
...
def generate_wins_draws_losses(wins, draws, losses):
    if wins > 0 and losses == 0:
        ...
        wl_ratio = (
            f"{100.0 / (wins + draws + losses) * wins:.1f}" if losses > 0 else "100"
        )
    return f"{wins:>4}  {draws:>4}  {losses:>4}  {wl_ratio:>4}"


def text_table_bt_results(
    pair_results: List[Dict[str, Any]], stake_currency: str
) -> str:
    """
    ...
    return tabulate(
        output, headers=headers, floatfmt=floatfmt, tablefmt="orgtbl", stralign="right"
    )


def text_table_exit_reason(
    exit_reason_stats: List[Dict[str, Any]], stake_currency: str
) -> str:
    """
...
def text_table_tags(
    tag_type: str, tag_results: List[Dict[str, Any]], stake_currency: str
) -> str:
    """
    Generates and returns a text table for the given backtest data and the results dataframe
    :param pair_results: List of Dictionaries - one entry per pair + final TOTAL row
    ...
    output = [
        [
            t["key"] if t["key"] is not None and len(t["key"]) > 0 else "OTHER",
            t["trades"],
            t["profit_mean_pct"],
            t["profit_sum_pct"],
            t["profit_total_abs"],
...
def text_table_add_metrics(strat_results: Dict) -> str:
    if len(strat_results["trades"]) > 0:
        ...
        short_metrics = (
            [
                ...
                (
                    ...
                    fmt_coin(
                        strat_results["profit_total_long_abs"],
                ...
                (
                    ...
                    fmt_coin(
                        strat_results["profit_total_short_abs"],
                        strat_results["stake_currency"],
                    ),
                ),
            ]
        ...
        if "max_relative_drawdown" in strat_results:
            ...
            drawdown_metrics.append(
                (
                    ...
                    f"{strat_results['max_relative_drawdown']:.2%}",
                )
            )
        drawdown_metrics.extend(
            [
                ...
                (
                    "Drawdown high",
                    fmt_coin(
                        strat_results["max_drawdown_high"],
                        strat_results["stake_currency"],
                    ),
                ),
                (
                    "Drawdown low",
                    fmt_coin(
                        strat_results["max_drawdown_low"],
                        strat_results["stake_currency"],
                    ),
                ),
        ...
        entry_adjustment_metrics = (
            [
                (
                    ...
                    strat_results.get("canceled_trade_entries", "N/A"),
                ),
                (
                    "Canceled Entry Orders",
                    strat_results.get("canceled_entry_orders", "N/A"),
                ),
                (
                    "Replaced Entry Orders",
                    strat_results.get("replaced_entry_orders", "N/A"),
                ),
            ]
        ...
        # command stores these results and newer version of trapilot must be able to handle old
        ...
        metrics = [
            ...
            (
                "Starting balance",
                fmt_coin(
                    strat_results["starting_balance"], strat_results["stake_currency"]
                ),
            ),
            (
                "Final balance",
                fmt_coin(
                    strat_results["final_balance"], strat_results["stake_currency"]
                ),
            ),
            (
                "Absolute profit ",
                fmt_coin(
                    strat_results["profit_total_abs"], strat_results["stake_currency"]
                ),
            ),
            ("Total profit %", f"{strat_results['profit_total']:.2%}"),
            (
                "CAGR %",
                f"{strat_results['cagr']:.2%}" if "cagr" in strat_results else "N/A",
            ),
            (
                "Sortino",
                (
                    f"{strat_results['sortino']:.2f}"
                    if "sortino" in strat_results
                    else "N/A"
                ),
            ),
            (
                "Sharpe",
                (
                    f"{strat_results['sharpe']:.2f}"
                    if "sharpe" in strat_results
                    else "N/A"
                ),
            ),
            (
                "Calmar",
                (
                    f"{strat_results['calmar']:.2f}"
                    if "calmar" in strat_results
                    else "N/A"
                ),
            ),
            (
            ...
            (
                "Avg. stake amount",
                fmt_coin(
                    strat_results["avg_stake_amount"], strat_results["stake_currency"]
                ),
            ),
            (
                "Total trade volume",
                fmt_coin(
                    strat_results["total_volume"], strat_results["stake_currency"]
                ),
            ),
            *short_metrics,
            ("", ""),  # Empty line to improve readability
            ...
            (
                "Worst trade",
                f"{worst_trade['pair']} " f"{worst_trade['profit_ratio']:.2%}",
            ),
            (
                "Best day",
                fmt_coin(
                    strat_results["backtest_best_day_abs"],
                    strat_results["stake_currency"],
                ),
            ),
            (
                "Worst day",
                fmt_coin(
                    strat_results["backtest_worst_day_abs"],
                    strat_results["stake_currency"],
                ),
            ),
            (
            ...
            *entry_adjustment_metrics,
            ...
            (
                "Min balance",
                fmt_coin(strat_results["csum_min"], strat_results["stake_currency"]),
            ),
            (
                "Max balance",
                fmt_coin(strat_results["csum_max"], strat_results["stake_currency"]),
            ),
            *drawdown_metrics,
...
def show_backtest_result(
    strategy: str, results: Dict[str, Any], stake_currency: str, backtest_breakdown=[]
):
    """
    ...
    if results.get("results_per_enter_tag") is not None:
        table = text_table_tags(
            "enter_tag", results["results_per_enter_tag"], stake_currency
        )
    ...
    if exit_reasons:
        table = text_table_exit_reason(
            exit_reason_stats=exit_reasons, stake_currency=stake_currency
        )
        if isinstance(table, str) and len(table) > 0:
...
def show_backtest_results(config: Config, backtest_stats: BacktestResultType):
    ...
    if len(backtest_stats["strategy"]) > 0:
        ...
        table = text_table_strategy(
            backtest_stats["strategy_comparison"], stake_currency
        )
        print(
