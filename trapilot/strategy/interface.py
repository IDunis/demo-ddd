...
class IStrategy(ABC, HyperStrategyMixin):
    ...
    def __init__(self, config: Config) -> None:
        ...
        for attr_name in dir(self.__class__):
            ...
            for informative_data in informative_data_list:
                if (
                    timeframe_to_minutes(informative_data.timeframe)
    ...
    def confirm_trade_entry(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.trapilot.io/en/latest/strategy-advanced/
    ...
    def adjust_entry_price(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.trapilot.io/en/latest/strategy-callbacks/
    ...
    def feature_engineering_expand_all(
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        https://www.trapilot.io/en/latest/freqai-parameter-table/#feature-parameters

        https://www.trapilot.io/en/latest/freqai-feature-engineering/#defining-the-features
    ...
    def feature_engineering_standard(
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        https://www.trapilot.io/en/latest/freqai-feature-engineering
    ...
    def __informative_pairs_freqai(self) -> ListPairsWithTimeframes:
        ...
        if self.config.get("freqai", {}).get("enabled", False):
            ...
            corr_pairs = self.config["freqai"]["feature_parameters"][
            ...
            if inf_data.asset:
                if any(s in inf_data.asset for s in ("{BASE}", "{base}")):
                    for pair in self.dp.current_whitelist():
                        ...
                        pair_tf = (
                            _format_pair_name(
                                self.config, inf_data.asset, self.dp.market(pair)
                            ),
                            inf_data.timeframe,
                            candle_type,
                        )
    ...
    def unlock_pair(self, pair: str) -> None:
        ...
        Not used by trapilot itself, but intended to be used if users lock pairs
    ...
    def _analyze_ticker_internal(
        ...
        new_candle = (
            self._last_candle_seen_per_pair.get(pair, None)
        ...
        if not self.process_only_new_candles or new_candle:
            ...
            self.dp._set_cached_df(
                pair, self.timeframe, dataframe, candle_type=candle_type
            )
    ...
    def assert_df(
        self, dataframe: DataFrame, df_len: int, df_close: float, df_date: datetime
    ):
        """
    ...
    def get_exit_signal(
        ...
        logger.debug(
            f"exit-trigger: {latest['date']} (pair={pair}) "
    ...
    def get_entry_signal(
        ...
        enter_tag = (
            enter_tag if isinstance(enter_tag, str) and enter_tag != "nan" else None
        )
        ...
        *,
        ...
        if self.use_exit_signal:
            if exit_ and not enter:
                ...
                reason_cust = strategy_safe_wrapper(
            ...
            if exit_signal == ExitType.CUSTOM_EXIT or (
                exit_signal == ExitType.EXIT_SIGNAL
                and (
                    not self.exit_profit_only
                    or current_profit > self.exit_profit_offset
                )
            ):
                logger.debug(
    ...
    def ft_stoploss_reached(
        ...
        if liq_higher_long or liq_lower_short:
            logger.debug(
                f"{trade.pair} - Liquidation price hit. exit_type=ExitType.LIQUIDATION"
            )
    ...
    def ft_check_timed_out(
        ...
        if timeout is not None:
            ...
            timedout = (
                order.status == "open" and order.order_date_utc < timeout_threshold
            )
            if timedout:
                return True
        time_method = (
