...
from trapilot.configuration.directory_operations import (create_datadir,
                                                         create_userdata_dir)
from trapilot.configuration.environment_vars import enironment_vars_to_dict
from trapilot.configuration.load_config import load_file, load_from_files
from trapilot.constants import Config
from trapilot.enums import (NON_UTIL_MODES, TRADING_MODES, CandleType, RunMode,
                            TradingMode)
from trapilot.exceptions import OperationalException
from trapilot.loggers import setup_logging
from trapilot.misc import deep_merge_dicts, parse_db_uri_for_logging
...
class Configuration:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        from trapilot.commands.arguments import NO_CONF_ALLOWED
        ...
        from trapilot.exchange.check_exchange import check_exchange

        # Check if the exchange set by the user is supported
        check_exchange(
            config, config.get("experimental", {}).get("block_bad_exchanges", True)
        )
    ...
    def _process_datadir_options(self, config: Config) -> None:
        ...
        config["user_data_dir"] = create_userdata_dir(
            config["user_data_dir"], create_dir=False
        )
    ...
    def _process_optimize_options(self, config: Config) -> None:
        ...
        if (
            "use_max_market_positions" in self.args
            and not self.args["use_max_market_positions"]
        ):
            ...
            logger.info(
                ...
                "overriding max_open_trades to: %s ...",
        ...
        if "stoploss_range" in self.args and self.args["stoploss_range"]:
            ...
            logger.info(
                "Parameter --stoplosses detected: %s ...", self.args["stoploss_range"]
            )
        ...
        configurations = [
            ...
            (
                ...
                "Saving lookahead analysis results into {} ...",
            ),
            (
                "epochs",
                "Parameter --epochs detected ... Will run Hyperopt with for {} epochs ...",
            ),
        ...
        configurations = [
            ...
            (
                ...
                "Parameter --min-total-profit detected: {}",
            ),
            (
                "hyperopt_list_max_total_profit",
                "Parameter --max-total-profit detected: {}",
            ),
            ...
            (
                ...
                "Paramter --ignore-missing-space detected: {}",
            ),
        ]
    ...
    def _process_analyze_options(self, config: Config) -> None:
        configurations = [
            ...
            (
                "lookahead_analysis_exportfilename",
                "Path to store lookahead-analysis-results: {}",
            ),
            ("startup_candle", "Startup candle to be used on recursive analysis: {}"),
        ]
        self._args_to_config_loop(config, configurations)

    def _args_to_config_loop(
        self, config, configurations: List[Tuple[str, str]]
    ) -> None:
    ...
    def _process_runmode(self, config: Config) -> None:
        ...
        if not self.runmode:
            ...
            self.runmode = (
                RunMode.DRY_RUN if config.get("dry_run", True) else RunMode.LIVE
            )
