...
from trapilot.deployment.deploy import zip_dir, get_python_version
from trapilot.deployment.keys import add_key, load_keys, write_keys
from trapilot.deployment.login import logout, poll_login, get_token
from trapilot.deployment.ui import text, confirm, print_work, print_failure, print_success, select, show_spinner, path
from trapilot.deployment.exchange_data import EXCHANGES, Exchange, EXCHANGE_CHOICES, exc_display_name, \
    EXCHANGE_CHOICES_NO_KEYLESS
from trapilot.utils.gc_setup import gc_set_threshold
from trapilot.utils.utils import load_deployment_settings, load_user_preferences, load_backtest_preferences
...
def trapilot_init(args):
    ...
    if exchange:
        ...
        if confirm('Would you like to add keys for this exchange?\n'
                   'You can do this later at any time by running `trapilot key add`').unsafe_ask():
    ...
    with show_spinner('Generating files') as spinner:
        files = [
            ...
            ('user_data/trapilot.json', generate_trapilot_json(api, model, model_type), False),
    ...
    print_success('Done! Your model was created. Run `python tradebot.py` to run a backtest and get started.')
...
def generate_trapilot_json(api: Optional[API], model: Optional[dict], model_type: str, main_script: str = 'bot.py'):
...
def missing_deployment_files() -> list:
    paths = ['user_data/trapilot.json', 'user_data/keys.json', 'user_data/backtest.json', 'user_data/settings.json', 'requirements.txt']
...
def main():
    parser = argparse.ArgumentParser(prog='trapilot', description='Trapilot CLI & deployment tool')
    ...
    monitor_parser = subparsers.add_parser('monitor', help='Process must be killed to halt monitoring')
    monitor_parser.set_defaults(func=start_monitor)

    monitor_parser = subparsers.add_parser('trade', help='Main entry point for trading mode')
