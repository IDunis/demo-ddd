...
class AlpacaInterface(ExchangeInterface):
    ...
    def init_exchange(self):
        ...
        try:
            if account_info["account_blocked"]:
                warnings.warn(
                    "Your alpaca account is indicated as blocked for trading...."
                )
        except KeyError:
    ...
    def get_products(self) -> dict:
        ...
        for asset in assets:
            ...
            if asset["fractionable"]:
                asset["base_min_size"] = 0.000000001
                asset["base_increment"] = 0.000000001
            else:
    ...
    def get_account(self, symbol=None):
        ...
        for position in positions:
            ...
            positions_dict[curr_symbol] = utils.AttributeDict(
                {"available": float(position.pop("qty")), "hold": 0.0}
            )
        ...
        positions_dict["USD"] = utils.AttributeDict(
            {"available": float(account["buying_power"]), "hold": 0.0}
        )
            ...
            if order["side"] == "buy":  # buy orders only affect USD holds
                if order["qty"]:  # this case handles qty market buy and limit buy
                    if order["type"] == "limit":
                        ...
                        dollar_amt = (
                            float(order["qty"])
                            * snapshot_price[curr_symbol]["latestTrade"]["p"]
                        )
        ...
        for i in self.__unique_assets:
            if i not in positions_dict:
                positions_dict[i] = utils.AttributeDict({"available": 0.0, "hold": 0.0})
    ...
    def __parse_iso(response):
        ...
        try:
            ...
            if str(e) == "Unused components in ISO string":
                response["created_at"] = parser.parse(
                    response["created_at"]
                ).timestamp()
            else:
                raise e
    ...
    def market_order(self, symbol, side, size) -> MarketOrder:
        ...
        response = self.calls.submit_order(
            symbol, side=side, type="market", time_in_force="day", qty=size
        )
    ...
    def _fix_response(self, needed, response):
        ...
        response = utils.rename_to(
            [["limit_price", "price"], ["qty", "size"]], response
        )
    ...
    def homogenize_order(self, order):
        if order["type"] == "limit":
            renames = [["qty", "size"], ["limit_price", "price"]]
    ...
    def get_fees(self, symbol):
        assert isinstance(self.calls, alpaca_trade_api.REST)
        return {"maker_fee_rate": 0.0, "taker_fee_rate": 0.0}
    ...
    def get_product_history(
        ...
        if not self.user_preferences["settings"]["alpaca"]["use_yfinance"]:
            ...
            if resolution not in supported_multiples:
                ...
                resolution = supported_multiples[
                    min(
                        range(len(supported_multiples)),
                        key=lambda i: abs(supported_multiples[i] - resolution),
                    )
                ]

            found_multiple, row_divisor = super().evaluate_multiples(
                supported_multiples, resolution
            )
            ...
            try:
                ...
                if e.code == 42210000:
                    warning_string = (
                        "Your alpaca subscription does not permit querying data from the last 15 "
                        "minutes. Trapilot is adjusting your query."
                    )
                    utils.info_print(warning_string)
                    epoch_stop = time.time() - (build_minute() * 15)
                    if epoch_stop >= epoch_start:
                        try:
                            return self.get_product_history(
    ...
    def overridden_history(
        self, symbol, epoch_start, epoch_stop, resolution_seconds, **kwargs
    ) -> pd.DataFrame:
        to = kwargs["to"]
        ...
        if to:
            if not self.user_preferences["settings"]["alpaca"]["use_yfinance"]:
                resolution_seconds = self.valid_resolutions[
                    min(
                        range(len(self.valid_resolutions)),
                        key=lambda j: abs(
                            self.valid_resolutions[j] - resolution_seconds
                        ),
                    )
                ]
                resolution_lookup = {
                    ...
                    while epoch_start_ <= epoch_stop_:
                        ...
                        frames.append(
                            self.calls.get_bars(
                                ...
                                time_interval,
                                limit=10000,
                                start=utils.iso8601_from_epoch(epoch_start_),
                            ).df
                        )
                ...
                while len(response) < to:
                    response = pd.concat(
                        [
                            response,
                            find_last_n_points(batched_ranges[0], batched_ranges[1]),
                        ]
                    )
                ...
                return (
                    self.parse_yfinance(
                        symbol, epoch_start, epoch_stop, resolution_seconds
                    )
                    .tail(to)
                    .reset_index()
                )
        else:
            response = self.get_product_history(
                symbol, epoch_start, epoch_stop, int(resolution_seconds)
            )
    ...
    def parse_yfinance(
        symbol: str,
        epoch_start: [int, float],
        epoch_stop: [int, float],
        resolution: int,
    ):
        try:
            import yfinance
            ...
            result = ticker.history(
                ...
                interval=number_interval_to_string(resolution),
            )
            ...
            raise ImportError(
                "To use yfinance to download data please pip install yfinance"
            )
