...
def load_auth(exchange_type, keys_file=None, name=None):
    ...
    if keys_file is None:
        if keys_path_cache is None:
            ...
            keys_path_cache = "./user_data/keys.json"
...
def __determine_first_key(exchange_keys):
    ...
    warning_string = (
        "No portfolio name to load specified, defaulting to the first in the file: "
...
def load_json(keys_file):
    try:
        ...
        raise FileNotFoundError(
            "Make sure a Keys.json file is placed in the same folder as the project working "
            "directory!"
        )
