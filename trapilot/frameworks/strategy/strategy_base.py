...
class StrategyBase:
    ...
    def __init__(self, exchange: ABCBaseExchange, interface: ABCBaseExchangeInterface, model):
        ...
        Create a new strategy object. A strategy can be used to run your code live while be backtestable and modular
         across exchanges.
         Args:
             exchange: An exchange object. This can be created by doing something similar to exchange = trapilot.Alpaca()

        Function Signatures:
        init(symbol: str, state: trapilot.StrategyState)
        price_event(price: float, symbol: str, state: trapilot.StrategyState)
        orderbook_event(tick: dict, symbol: str, state: trapilot.StrategyState)
        bar_event(bar: dict, symbol: str, state: trapilot.StrategyState)
        teardown(trapilot.StrategyState)
        """
        self.remote_backtesting = trapilot._backtesting
    ...
    def __custom_price_event(self,
        ...
        if symbol is not None:
            if isinstance(symbol, list):
                for i in symbol:
                    trapilot.reporter.export_used_symbol(i)
            else:
                trapilot.reporter.export_used_symbol(symbol)
