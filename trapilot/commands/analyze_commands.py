...
def setup_analyze_configuration(
            ...
            if config["exportfilename"].is_dir():
                ...
                if config["exportfilename"].exists():
                    ...
                    raise OperationalException(
                        f"{config['exportfilename']} does not exist."
                    )
        else:
...
def start_analysis_entries_exits(args: Dict[str, Any]) -> None:
    ...
    :return: None
    """
    from trapilot.data.entryexitanalysis import process_entry_exit_reasons

    # Initialize configuration
    config = setup_analyze_configuration(args, RunMode.BACKTEST)

    logger.info("Starting trapilot in analysis mode")
