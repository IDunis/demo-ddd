...
from trapilot.configuration.directory_operations import chown_user_directory
from trapilot.constants import UNLIMITED_STAKE_AMOUNT
from trapilot.exceptions import OperationalException
from trapilot.exchange import MAP_EXCHANGE_CHILDCLASS, available_exchanges
from trapilot.util import render_template
...
def ask_user_config() -> Dict[str, Any]:
    ...
    answers["margin_mode"] = (
        "isolated" if answers.get("trading_mode") == "futures" else ""
    )
...
def deploy_new_config(config_path: Path, selections: Dict[str, Any]) -> None:
    ...
    config_text = render_template(
        templatefile="base_config.json.j2", arguments=selections
    )
