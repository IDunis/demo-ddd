...
from trapilot.constants import (CANCELED_EXCHANGE_STATES,
                                CUSTOM_TAG_MAX_LENGTH, DATETIME_PRINT_FORMAT,
                                MATH_CLOSE_PREC, NON_OPEN_EXCHANGE_STATES,
                                BuySell, LongShort)
...
class Configuration(ModelBase):
    ...
    unfilledtimeout: Mapped[Optional[str]] = mapped_column(
        String(255),
        default=json.dumps(
            {"entry": 10, "exit": 10, "exit_timeout_count": 0, "unit": "minutes"}
        ),
    )
    entry_pricing: Mapped[Optional[str]] = mapped_column(
        String(255),
        default=json.dumps(
            {
                "price_side": "same",
    ...
    exit_pricing: Mapped[Optional[str]] = mapped_column(
        String(255),
        default=json.dumps(
    ...
    pairlists: Mapped[Optional[str]] = mapped_column(
        String(255), default="[" + json.dumps({"method": "StaticPairList"}) + "]"
    )
    telegram: Mapped[Optional[str]] = mapped_column(
        String(255), default=json.dumps({"enabled": True, "token": "", "chat_id": ""})
    )
    api_server: Mapped[Optional[str]] = mapped_column(
        String(255),
        default=json.dumps(
            {
                "enabled": False,
...
class Order(ModelBase):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    ft_trade_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("trades.id"), index=True
    )

    _trade_live: Mapped["Trade"] = relationship(
        "Trade", back_populates="orders", lazy="immediate"
    )
    ...
    ft_cancel_reason: Mapped[str] = mapped_column(
    ...
    def order_filled_utc(self) -> Optional[datetime]:
        ...
        return (
            self.order_filled_date.replace(tzinfo=timezone.utc)
    ...
    def update_from_ccxt_object(self, order):
        ...
        self.order_type = safe_value_fallback(
            order, "type", default_value=self.order_type
        )
        ...
        self.remaining = safe_value_fallback(
            order, "remaining", default_value=self.remaining
        )
        self.cost = safe_value_fallback(order, "cost", default_value=self.cost)
        self.stop_price = safe_value_fallback(
            order, "stopPrice", default_value=self.stop_price
        )
            ...
            resp.update(
                {
                    ...
                    "order_timestamp": (
                        int(
                            self.order_date.replace(tzinfo=timezone.utc).timestamp()
                            * 1000
                        )
                        if self.order_date
                        else None
...
class LocalTrade:
    ...
    def borrowed(self) -> float:
        ...
        if self.has_no_leverage:
            ...
            return (self.amount * self.open_rate) * (
                (self.leverage - 1) / self.leverage
            )
        else:
            return self.amount
    ...
    def open_orders(self) -> List[Order]:
        ...
        """
        return [
            o for o in self.orders if o.ft_is_open and o.ft_order_side != "stoploss"
        ]
    ...
    def __repr__(self):
        open_since = (
            self.open_date_utc.strftime(DATETIME_PRINT_FORMAT)
            if self.is_open
            else "closed"
        )
    ...
    def to_json(self, minified: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            "amount_requested": (
                round(self.amount_requested, 8) if self.amount_requested else None
            ),
            "stake_amount": round(self.stake_amount, 8),
            "max_stake_amount": (
                round(self.max_stake_amount, 8) if self.max_stake_amount else None
            ),
            ...
            "close_date": (
                self.close_date.strftime(DATETIME_PRINT_FORMAT)
                if self.close_date
                else None
            ),
            ...
            "close_profit_pct": (
                round(self.close_profit * 100, 2) if self.close_profit else None
            ),
    ...
    def adjust_min_max_rates(
        self, current_price: float, current_price_low: float
    ) -> None:
        """
    ...
    def adjust_stop_loss(
        ...
        """
        if stoploss is None or (
            initial and not (self.stop_loss is None or self.stop_loss == 0)
        ):
    ...
    def update_trade(self, order: Order, recalculating: bool = False) -> None:
        ...
        if order.ft_order_side != self.entry_side:
            ...
            if isclose(
                order.safe_amount_after_fee, amount_tr, abs_tol=MATH_CLOSE_PREC
    ...
    def calculate_interest(self) -> FtPrecise:
        ...
        return interest(
            exchange_name=self.exchange, borrowed=borrowed, rate=rate, hours=hours
        )

    def _calc_base_close(
        self, amount: FtPrecise, rate: float, fee: Optional[float]
    ) -> FtPrecise:
    ...
    def calc_close_trade_value(
        ...
        if trading_mode == TradingMode.SPOT:
            ...
            if self.is_short:
                ...
                return float(
                    self._calc_base_close(amount1, rate, self.fee_close)
                    - total_interest
                )
            ...
            if self.is_short:
                return (
                    float(self._calc_base_close(amount1, rate, self.fee_close))
                    - funding_fees
                )
            else:
                return (
            ...
            raise OperationalException(
                f"{self.trading_mode.value} trading is not yet available using trapilot"
            )
    ...
    def calculate_profit(
        ...
        try:
            if self.is_short:
                profit_ratio = (
                    1 - (close_trade_value / open_trade_value)
                ) * self.leverage
            else:
                profit_ratio = (
                    (close_trade_value / open_trade_value) - 1
                ) * self.leverage
    ...
    def recalc_trade_from_orders(self, *, is_closing: bool = False):
        ...
        for i, o in enumerate(self.orders):
            ...
            if is_exit:
                ...
                total_stake = total_stake + self._calc_open_trade_value(
                    tmp_amount, price
                )
    ...
    def select_order(
        ...
        if is_open is False and only_filled:
            orders = [
                o for o in orders if o.filled and o.status in NON_OPEN_EXCHANGE_STATES
            ]
        if len(orders) > 0:
            return orders[-1]
        else:
    ...
    def get_trades_proxy(
        *,
        ...
        if close_date:
            sel_trades = [
                trade
                for trade in sel_trades
                if trade.close_date and trade.close_date > close_date
            ]
    ...
    def stoploss_reinitialization(desired_stoploss: float):
        ...
        for trade in Trade.get_open_trades():
            ...
            if (
                not trade.is_stop_loss_trailing
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        trade = cls(
            ...
            open_date=datetime.fromtimestamp(
                data["open_timestamp"] // 1000, tz=timezone.utc
            ),
        ...
        for order in data["orders"]:
            ...
            order_obj = Order(
                ...
                order_date=datetime.strptime(
                    order["order_date"], DATETIME_PRINT_FORMAT
                ),
                order_filled_date=(
...
class Trade(ModelBase, LocalTrade):
    ...
    @staticmethod
    def get_trades(
        trade_filter=None, include_orders: bool = True
    ) -> ScalarResult["Trade"]:
        """
        Helper function to query Trades using filters.
        NOTE: Not supported in Backtesting.
                    ...
                    while i < len(resp):
                        if resp[i]["mix_tag"] == mix_tag:
                            resp[i] = {
                                ...
                                "profit_pct": round(
                                    profit + resp[i]["profit_ratio"] * 100, 2
                                ),
