...
from trapilot.constants import (BACKTEST_BREAKDOWNS, DATETIME_PRINT_FORMAT,
                                IntOrInf)
from trapilot.data.metrics import (calculate_cagr, calculate_calmar,
...
def _generate_result_line(
    result: DataFrame, starting_balance: int, first_column: str
) -> Dict:
    """
    ...
    return {
        ...
        "profit_mean_pct": (
            result["profit_ratio"].mean() * 100.0 if len(result) > 0 else 0.0
        ),
        ...
        "winrate": (
            len(result[result["profit_abs"] > 0]) / len(result) if len(result) else 0.0
        ),
    }
...
def generate_pair_metrics(
    ...
    tabular_data = sorted(
        tabular_data, key=lambda k: k["profit_total_abs"], reverse=True
    )
...
def generate_exit_reason_stats(
    max_open_trades: IntOrInf, results: DataFrame
) -> List[Dict]:
    """
        ...
        tabular_data.append(
            {
                ...
                "losses": len(result[result["profit_abs"] < 0]),
                "winrate": (
                    len(result[result["profit_abs"] > 0]) / count if count else 0.0
                ),
...
def generate_periodic_breakdown_stats(
    ...
    results = (
        ...
        else DataFrame.from_records(trade_list)
    )
    if len(results) == 0:
        return []
...
def calc_streak(dataframe: DataFrame) -> Tuple[int, int]:
    ...
    df = Series(np.where(dataframe["profit_ratio"] > 0, "win", "loss")).to_frame(
        "result"
    )
...
def generate_strategy_stats(
    ...
    if not is_hyperopt:
        periodic_breakdown = {
            "periodic_breakdown": generate_all_periodic_breakdown_stats(results)
        }
    ...
    strat_stats = {
        ...
        "profit_total_long": results.loc[~results["is_short"], "profit_abs"].sum()
        / start_balance,
        "profit_total_short": results.loc[results["is_short"], "profit_abs"].sum()
        / start_balance,
