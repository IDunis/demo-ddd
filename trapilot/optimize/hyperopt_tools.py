...
from trapilot.constants import FTHYPT_FILEVERSION, Config
from trapilot.enums import HyperoptState
from trapilot.exceptions import OperationalException
from trapilot.misc import deep_merge_dicts, round_dict, safe_value_fallback2
from trapilot.optimize.hyperopt_epoch_filters import hyperopt_filter_epochs
from trapilot.optimize.optimize_reports import generate_wins_draws_losses
from trapilot.util import fmt_coin
...
class HyperoptTools:
    ...
    def try_export_params(config: Config, strategy_name: str, params: Dict):
        if params.get(FTHYPT_FILEVERSION, 1) >= 2 and not config.get(
            ...
            if fn:
                HyperoptTools.export_params(
                    params, strategy_name, fn.with_suffix(".json")
                )
            else:
        ...
        for epochs_tmp in HyperoptTools._read_results(results_file):
            if total_epochs == 0 and epochs_tmp[0].get("is_best") is None:
                raise OperationalException(
                    ...
                    "of Trapilot and cannot be loaded."
                )
    ...
    def show_epoch_details(
        ...
        if not no_header:
            explanation_str = HyperoptTools._format_explanation_string(
        ...
        if print_json:
            ...
            for s in [
                ...
                HyperoptTools._params_update_for_json(
                    result_dict, params, non_optimized, s
                )
            print(
                rapidjson.dumps(
                    result_dict, default=str, number_mode=HYPER_PARAMS_FILE_FORMAT
                )
            )
            ...
            HyperoptTools._params_pretty_print(
                params, "roi", "ROI table:", non_optimized
            )
            HyperoptTools._params_pretty_print(
                params, "stoploss", "Stoploss:", non_optimized
            )
            HyperoptTools._params_pretty_print(
                params, "trailing", "Trailing stop:", non_optimized
            )
            HyperoptTools._params_pretty_print(
    ...
    @staticmethod
    def format_results_explanation_string(
        results_metrics: Dict, stake_currency: str
    ) -> str:
        """
    ...
    @staticmethod
    def prepare_trials_columns(
        trials: pd.DataFrame, has_drawdown: bool
    ) -> pd.DataFrame:
        trials["Best"] = ""
        ...
        trials["results_metrics.winsdrawslosses"] = trials.apply(
            lambda x: generate_wins_draws_losses(
                x["results_metrics.wins"],
                x["results_metrics.draws"],
    ...
    def get_result_table(
        ...
        trials["Avg duration"] = trials["Avg duration"].apply(
            lambda x: (
                ...
                else f"{x}" if not isna(x) else "--".rjust(7, " ")
            )
        )
        ...
        trials[f"Max Drawdown{' (Acct)' if has_account_drawdown else ''}"] = (
            trials.apply(
                lambda x: (
                    "{} {}".format(
                        fmt_coin(
        ...
        trials = trials.drop(
            columns=["max_drawdown_abs", "max_drawdown", "max_drawdown_account"]
        )
        ...
        trials["Profit"] = trials.apply(
            lambda x: (
                "{} {}".format(
                    fmt_coin(
                        x["Total profit"], stake_currency, keep_trailing_zeros=True
                    ),
        ...
        if print_colorized:
            ...
            for i in range(len(trials)):
                if trials.loc[i]["is_profit"]:
                    for j in range(len(trials.loc[i]) - 3):
                        trials2.iat[i, j] = (
                            f"{Fore.GREEN}{str(trials.iloc[i, j])}{Fore.RESET}"
                        )
            ...
            del trials2
        trials = trials.drop(
            columns=["is_initial_point", "is_best", "is_profit", "is_random"]
        )
        if remove_header > 0:
    ...
    def export_csv_file(config: Config, results: list, csv_file: str) -> None:
        ...
        param_metrics = [
            ("params_dict." + param) for param in results[0]["params_dict"].keys()
        ]
