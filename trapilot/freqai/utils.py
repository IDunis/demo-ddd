...
from trapilot.data.history.history_utils import refresh_backtest_ohlcv_data
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    indicator_periods = config["freqai"]["feature_parameters"][
    ...
    startts = int(
        time - config["freqai"].get("train_period_days", 0) * 86400 - additional_seconds
    )
    stopts = int(time)
...
def plot_feature_importance(
    ...
    """
    from trapilot.plot.plotting import go, make_subplots, store_plot_file
    ...
    for label in models:
        ...
        label = label.replace("&", "").replace(
            "%", ""
        )  # escape two FreqAI specific characters
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    ...
    if model_type == "pytorch" and activate:
        from trapilot.freqai.tensorboard import TBLogger
